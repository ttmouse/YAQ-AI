...
(function () {
    ...
    },
    {
      ...
      build: function () {
        return {
          ...
          condition: {
            type: 'behavior',
            params: { target: 'selfCheck', operator: 'eq', value: 0, unit: '次', window: '本期' },
          },
          effect: { markAlert: 'warning', generatePriority: false },
        };
      },
  ...
  var defaultRules = [
    ...
    },
    {
      ...
      stats: {
        hitCount: 18,
        lastHit: '2025-07-21 09:15',
        recentHits: [{ time: '2025-07-21 09:15', context: '今日监管工作台', detail: '关闭率 91%' }],
      },
    },
    {
      ...
      stats: {
        hitCount: 30,
        lastHit: '2025-07-21 09:15',
        recentHits: [{ time: '2025-07-21 09:15', context: '今日监管工作台', detail: '未闭环重大隐患 2 项' }],
      },
    },
    ...
    },
    {
      ...
      stats: {
        hitCount: 5,
        lastHit: '2025-07-20 10:00',
        recentHits: [{ time: '2025-07-20 10:00', context: '重大隐患整改日报', detail: '2 项重大隐患超期' }],
      },
    },
    ...
    // ── 对比类 ──
    {
      ...
      condition: {
        type: 'comparison',
        params: { target: 'linePerformance', operator: 'lt', value: 15, unit: 'pp', compareTo: 'peerAverage' },
      },
      effect: { markAlert: 'warning', generatePriority: false, label: '偏低' },
      stats: {
        hitCount: 6,
        lastHit: '2025-07-20 14:00',
        recentHits: [{ time: '2025-07-20 14:00', context: '履职效能分析', detail: '张毅条线低于均值 17%' }],
      },
    },
    {
      id: 'rule-comp-self',
      ...
      condition: {
        type: 'comparison',
        params: { target: 'enterpriseRisk', operator: 'gt', value: 200, unit: '%', compareTo: 'peerAverage' },
      },
    ...
    // ── 趋势类 ──
    {
      ...
      condition: {
        type: 'trend',
        params: { target: 'newHazard', operator: 'gt', value: 20, unit: '%', period: '环比' },
      },
      effect: { markAlert: 'warning', generatePriority: true, label: '上升' },
      stats: {
        hitCount: 7,
        lastHit: '2025-07-21 09:15',
        recentHits: [{ time: '2025-07-21 09:15', context: '今日监管工作台', detail: '隐患新增 ↑28%' }],
      },
    },
    {
      ...
      condition: {
        type: 'trend',
        params: { target: 'closureRate', operator: 'lt', value: 0, unit: '', period: '连续下降', consecutive: 2 },
      },
    ...
    // ── 行为模式 ──
    {
      ...
      condition: {
        type: 'behavior',
        params: { target: 'enterpriseLogin', operator: 'gte', value: 30, unit: '天', window: '连续' },
      },
    ...
    // ── 复合规则 ──
    {
      ...
      condition: {
        type: 'composite',
        params: { rules: ['rule-timeout-general', 'rule-behavior-login'], logic: 'AND' },
      },
  ...
  function persistRuleEvaluation(ruleIdx, result) {
    ...
    rules[ruleIdx].stats.recentHits.unshift({
      time: getNow(),
      context: getTriggerContext(rules[ruleIdx]),
  ...
  function renderRulesPage() {
    ...
    html +=
      '    <span class="rules-ai-toggle-icon"><i data-lucide="chevron-' +
      (aiZoneVisible ? 'up' : 'down') +
    ...
    if (aiZoneVisible) {
      ...
      for (var i = 0; i < Math.min(examples.length, 4); i++) {
        html +=
          ' <span class="rules-ai-hint" onclick="document.getElementById(\'aiRuleInput\').value=this.textContent;window.parseAiRule()">' +
          escHtml(examples[i]) +
          '</span>';
      ...
      if (aiPreviewRule) {
        ...
        html +=
          ...
          aiPreviewRule.severity +
          '">' +
          severityLabel(aiPreviewRule.severity) +
          '</span>';
        ...
        html +=
          '    <button class="rules-btn rules-btn-primary" onclick="window.confirmAiRule()"><i data-lucide="check" aria-hidden="true"></i> 确认添加</button>';
        html +=
          '    <button class="rules-btn" onclick="window.dismissAiRule()"><i data-lucide="x" aria-hidden="true"></i> 取消</button>';
        html += '  </div>';
        html += '</div>';
      }
      html += '  </div>';
    ...
    for (var d = 0; d < DIMENSIONS.length; d++) {
      ...
      html +=
        '  <div class="rules-dim-item' +
        active +
        '" data-dim="' +
        dim.id +
        '" onclick="window.switchRulesDim(\'' +
        dim.id +
        '\')">';
    ...
    html +=
      '    <div class="rules-log-toggle">' +
      (hitLogVisible ? '收起' : '展开') +
      ' <i data-lucide="chevron-' +
      (hitLogVisible ? 'up' : 'down') +
    ...
    if (hitLogVisible) {
      ...
      if (logEntries.length === 0) {
        ...
        for (var l = 0; l < Math.min(logEntries.length, 20); l++) {
          ...
          html +=
            '      <div class="rules-log-sev"><span class="badge-sev badge-' +
            entry.severity +
            '">' +
  ...
  function renderRuleCard(rule) {
    ...
    html +=
      '      <span><i data-lucide="activity" aria-hidden="true" style="width:11px;height:11px"></i> 命中 ' +
      rule.stats.hitCount +
      ' 次</span>';
    html +=
      '      <span><i data-lucide="clock" aria-hidden="true" style="width:11px;height:11px"></i> ' +
      (rule.stats.lastHit || '从未命中') +
      '</span>';
    html += '    </div>';
    html += '    <div class="rules-card-actions">';
    html +=
      '      <button class="rules-btn rules-btn-xs" onclick="window.viewRuleDetail(\'' +
      rule.id +
      '\')"><i data-lucide="eye" aria-hidden="true"></i> 详情</button>';
    html += '    </div>';
    html += '  </div>';
    ...
    if (rule.stats.recentHits && rule.stats.recentHits.length > 0) {
      ...
      for (var h = 0; h < Math.min(rule.stats.recentHits.length, 3); h++) {
        ...
        html +=
          '    <div class="rules-card-hit"><span class="rules-hit-time">' +
          hit.time +
          '</span> <span class="rules-hit-ctx">' +
          escHtml(hit.context) +
          '</span> <span class="rules-hit-detail">' +
          escHtml(hit.detail) +
          '</span></div>';
      }
      html += '  </div>';
  ...
  function viewRuleDetail(ruleId) {
    ...
    html +=
      '    <button class="rules-detail-close" onclick="window.closeRuleDetail()"><i data-lucide="x" aria-hidden="true"></i></button>';
    html += '  </div>';
    ...
    html +=
      '        <div class="rules-detail-field"><span class="rules-detail-label">描述</span><span>' +
      escHtml(rule.description) +
      '</span></div>';
    html +=
      '        <div class="rules-detail-field"><span class="rules-detail-label">维度</span><span>' +
      getDimLabel(rule.dimension) +
      '</span></div>';
    html +=
      '        <div class="rules-detail-field"><span class="rules-detail-label">严重等级</span><span class="badge-sev badge-' +
      rule.severity +
      '">' +
      severityLabel(rule.severity) +
      '</span></div>';
    html +=
      '        <div class="rules-detail-field"><span class="rules-detail-label">当前状态</span><span>' +
    ...
    html +=
      '      <div class="rules-detail-tip">💡 你也可以对 AI 说：<strong>"修改此规则阈值到 80"</strong> 来调整</div>';
    html += '    </div>';
    html += '    <div class="rules-detail-section">';
    ...
    html += '      <div class="rules-detail-grid">';
    html +=
      '        <div class="rules-detail-field"><span class="rules-detail-label">总命中次数</span><span>' +
      rule.stats.hitCount +
      ' 次</span></div>';
    html +=
      '        <div class="rules-detail-field"><span class="rules-detail-label">最近命中</span><span>' +
      (rule.stats.lastHit || '无') +
      '</span></div>';
    html += '      </div>';
    html += '    </div>';
    html += '  </div>';
  ...
  function renderRulesList() {
    ...
    if (filteredRules.length === 0) {
      html +=
        '<div class="rules-empty"><i data-lucide="file-text" aria-hidden="true" style="color:var(--weak);width:32px;height:32px;margin-bottom:8px"></i><div>暂无此维度规则</div><div class="rules-empty-hint">展开上方 AI 配置快速添加</div></div>';
    } else {
  ...
  function updateSummaryCount() {
    ...
    el.textContent =
      '共 ' + rules.length + ' 条规则 · ' + enabledCount + ' 条启用 · 本日命中 ' + getTodayHitCount() + ' 次';
  }
  ...
  function showRulesToast(msg) {
    ...
    t._hide = setTimeout(function () {
      t.classList.remove('rules-toast-show');
    }, 2500);
  }
  ...
  window.toggleRule = function (id, en) {
    toggleRule(id, en);
  };
  window.switchRulesDim = function (id) {
    switchRulesDim(id);
  };
  window.parseAiRule = function () {
    parseAiRule();
  };
  window.confirmAiRule = function () {
    confirmAiRule();
  };
  window.dismissAiRule = function () {
    dismissAiRule();
  };
  window.toggleHitLog = function () {
    toggleHitLog();
  };
  window.toggleAiZone = function () {
    toggleAiZone();
  };
  window.viewRuleDetail = function (id) {
    viewRuleDetail(id);
  };
  ...
  window.getRuleEngineSummary = function () {
    ...
    return {
      ...
      enabled: enabled,
      triggered: triggered,
      alerts: Object.keys(alerts).map(function (k) {
        return alerts[k];
      }),
    };
  };
