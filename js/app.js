(function () {
  ...
  function renderError(msg, desc) {
    return (
      '<div class="state-error"><div class="state-icon"><svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg></div><div class="state-title">' +
      (msg || '加载失败') +
      '</div>' +
      (desc ? '<div class="state-desc">' + desc + '</div>' : '') +
      '</div>'
      ...
      try {
        switch (sceneId) {
          case 'dashboard':
            ...
            break;
          case 'hazard-report':
            html = renderHazardReport();
            break;
          case 'efficiency':
            html = renderEfficiency();
            break;
          case 'responsibility':
            html = renderResponsibility();
            break;
          case 'disposal':
            html = renderDisposal();
            break;
          ...
          case 'pending-actions':
            html = renderPendingActions();
            break;
          case 'supervision-track':
            html = renderSupervisionTrack();
            break;
  ...
  function renderDashboard() {
    ...
    var baseMetrics = [
      ...
      },
      {
        ...
        group: '监管概况',
        type: '期间',
        periods: ['今日', '本周', '本月', '本季', '本年'],
        desc: '统计周期内被检查或监管触达过的主体对象数量',
      },
      {
        id: 'coverageRate',
        label: '主体覆盖率',
      ...
      // 重大隐患
      {
        ...
        drilldown: [
          {
            ...
            statusText: '新发现',
            statusCls: 'danger',
            detail: '消防通道被货物大面积占用，存在严重安全隐患',
            person: '王志安',
            foundDate: '2026-06-29',
            deadline: '2026-07-06',
            overdue: 0,
            source: '日常巡查',
            region: '良渚街道',
          },
          {
            ...
            statusText: '新发现',
            statusCls: 'danger',
            detail: '自动喷淋系统故障导致18-25层消防设施大面积失效',
            person: '李明',
            foundDate: '2026-06-29',
            deadline: '2026-07-06',
            overdue: 0,
        ...
        aiAnalysis: [
          {
            label: '趋势分析',
            text: '昨日新增0项→今日新增2项，环比上升200%。均为消防安全组管辖的公共聚集场所，涉及人员密集场所安全。',
          },
          {
            label: '风险推演',
            text: '若今日新增项未在48小时内启动整改，将与现有超期项叠加（消防安全组超期2项→4项），影响本月闭环率目标（当前77.7%→可能跌破70%）。',
          },
        ],
      },
      { id: 'majorCum', label: '累计重大隐患', value: '12', group: '重大隐患', type: '累计' },
      {
        ...
        type: '时点',
        alert: 'danger',
        desc: '截至当前仍未完成整改闭环的重大隐患数量',
        compare: { baselineLabel: '上周', baselineValue: '4', delta: '+1', isBad: true },
        drilldown: [
          {
            name: '北苑商业综合体',
            line: '消防安全组',
            ...
            statusText: '超期未整改',
            statusCls: 'danger',
            detail: '消防通道被货架和杂物严重堵塞，宽度不足1.2米',
            person: '王志安',
            foundDate: '2026-06-10',
            deadline: '2026-06-22',
            overdue: 3,
          ...
          },
          {
            ...
            statusText: '超期未整改',
            statusCls: 'danger',
            detail: '自动喷淋系统、烟感探测器大面积失效，覆盖18-25层',
            person: '李明',
            foundDate: '2026-06-15',
            deadline: '2026-06-24',
            overdue: 1,
          ...
          },
          {
            ...
            statusText: '整改推进中',
            statusCls: 'warning',
            detail: '危化品存储区警示标识缺失，未设置临时围挡和出入登记',
            person: '李安全',
            foundDate: '2026-06-20',
            deadline: '2026-07-26',
            overdue: 0,
          ...
          },
          {
            ...
            statusText: '整改推进中',
            statusCls: 'warning',
            detail: '近30天企业自查0次，政府检查发现隐患8项未整改',
            person: '张毅',
          ...
          },
          {
            ...
            statusText: '尚未启动整改',
            statusCls: 'neutral',
            detail: '多项隐患叠加：疏散通道堵塞、灭火器过期、电气线路私拉乱接',
            person: '陈芳',
            foundDate: '2026-06-22',
            deadline: '2026-07-15',
            overdue: 0,
        ...
        aiAnalysis: [
          {
            label: '关联分析',
            text: '消防安全组2项超期（北苑商业综合体逾期3天、云栖高层住宅逾期1天），与该组"复查闭环率68%（↓6pp）"数据关联——复查环节效率不足。该组人均日处理量估算为4.2项，当前日均新增+待复查量约6.8项/人，人力已超饱和约62%。建议排查复查人力配置或抽查任务排序。',
          },
          {
            label: '交叉验证',
            text: '北苑商业综合体消防通道堵塞为反复出现项（本月已发生第3次），与"重点监管主体异常"中该主体的记录吻合。单一主体的反复问题需从管理机制入手，建议约谈物业管理方而非仅单次整改。',
          },
          {
            label: '特征分析',
            text: '企业安全组3项（杭州恒源化工有限公司、杭州鑫盛机械制造有限公司、余杭天元纺织厂）均位于良渚片区，与"良渚片重大风险检查任务覆盖141家、完成率0%"数据吻合。同片区多家企业同时出问题，存在区域性风险集中特征。良渚片整体自查率仅43%，企业端配合度偏低。',
          },
        ],
      },
      {
      ...
      },
      {
        ...
        drilldown: [
          {
            ...
            statusCls: 'danger',
            detail:
              '新增隐患12项（环比+3），逾期未整改从1项增至4项，重大隐患2项均为本周新认定。杭州华阳包装材料有限公司自查持续为0，为新增重点监管主体。',
            person: '陈芳',
            region: '物流片',
            foundDate: '2026-06-24',
            deadline: '持续跟进',
          },
          {
            name: '良渚街道',
            ...
            statusCls: 'danger',
            detail:
              '新增隐患16项（环比+5），逾期未整改累积5项。企业安全组（杭州鑫盛机械制造有限公司、余杭天元纺织厂）和消防安全组（北苑商业综合体）同区多主体异常叠加。辖区自查率仅43%，企业端配合度偏低。',
            person: '王志安 / 张毅',
            region: '良渚街道',
            foundDate: '2026-06-22',
            deadline: '持续跟进',
        ...
        aiAnalysis: [
          {
            label: '特征分析',
            text: '2个风险上升片区类型不同：物流片为"自查缺失→检查漏出→逾期蔓延"的单线恶化；良渚为多主体、跨条线异常叠加，且自查率系统性偏低（43%），属于区域性管理缺失。',
          },
          {
            label: '关联分析',
            text: '物流片区组长陈芳同时出现在重大隐患列表（余杭天元纺织厂），该片区逾期从1项飙升至4项，且同期新增重大隐患，符合"风险上升片区"判定。良渚街道与"重点监管主体异常"的8家中4家重合，异常密度最高。',
          },
        ],
      },
      // 今日聚焦（站长每日必看）
      {
        ...
        aiAnalysis: [
          {
            label: '优先级建议',
            text: '3项均为今日到期，涉及3个责任主体。建议优先处理北苑商业综合体（消防通道堵塞）——该主体已有超期记录，若今日未完成将转为第2项超期，进一步拉低消防安全组指标。其次处理恒源化工（危化品标识验收），已有整改方案，验收通过概率较高。',
          },
          {
            label: '资源评估',
            text: '杭州永固建材有限公司为培训到期整改，可由企业自行完成线上培训后提交凭证，无需现场核查，建议作为"自行整改"处理以减轻一线人力压力。',
          },
        ],
      },
      {
        ...
        type: '时点',
        alert: 'danger',
        desc: '重点监管主体中存在风险上升、长期未登录、自查异常、隐患反复等异常的数量',
        drilldown: [
          {
            name: '恒源化工',
            line: '企业安全组',
        ...
        aiAnalysis: [
          {
            label: '特征分析',
            text: '8家异常主体中，5家属于企业安全组管辖的工贸企业（杭州恒源化工有限公司、杭州鑫盛机械制造有限公司、余杭天元纺织厂、杭州华阳包装材料有限公司、余杭东兴精密机械厂），共性特征为"自查持续为0"。其中4家自查为0的同时培训完成率低于40%，企业安全主体责任落实存在系统性缺失。',
          },
          {
            label: '关联分析',
            text: '自查缺失的企业——杭州鑫盛机械制造有限公司、余杭东兴精密机械厂、杭州华阳包装材料有限公司——同期政府检查隐患数分别为8项、5项、4项。自查0 vs 政府查出一大堆，"自查与检查差异"显著，企业可能是在敷衍自查或根本未开展。建议安排专项抽查核实。',
          },
          {
            label: '风险推演',
            text: '消防安全组3项异常（北苑、云栖、高层专项）均与高层建筑消防相关。考虑到当前消防安全组复查闭环率仅68%，若不增加该组支援力量，本月闭环率可能跌破65%的警戒线，触发上级督办。',
          },
        ],
      },
      {
      ...
      // 主体责任
      {
        ...
        periods: ['今日', '本周', '本月'],
        alert: 'warning',
        desc: '统计周期内风险等级上升或风险指标明显变差的主体对象数量',
      },
      {
        id: 'selfCheckAbnormal',
        label: '自查异常主体数',
        value: '5',
        group: '主体责任',
        type: '时点',
        alert: 'warning',
        desc: '未按要求自查、长期不上报或自查质量异常的主体数量',
      },
      {
        id: 'selfCheckDiff',
        label: '自查与检查差异主体数',
      ...
      },
      {
        ...
        group: '主体责任',
        type: '期间',
        periods: ['本月', '本季', '本年', '近30天'],
        alert: 'warning',
        desc: '反复出现同类隐患或整改后复发的主体对象数量',
      },
      // 履职效能
      {
        id: 'staffAbnormal',
        ...
        periods: ['今日', '本周', '本月'],
        alert: 'warning',
        desc: '任务完成率低、隐患发现率异常低、复查闭环慢的人员或小组数量',
      },
      {
        id: 'expertAbnormal',
        label: '专家履职异常数',
      ...
      // 风险结构
      {
        ...
        group: '风险结构',
        type: '期间',
        periods: ['近7天', '今日', '本周', '本月', '本季', '本年'],
        desc: '统计周期内出现频次最高的隐患类型排行',
      },
      // 专项任务
      {
      ...
      // 执法处置
      {
        ...
        group: '执法处置',
        type: '期间',
        periods: ['今日', '本周', '本月', '本季', '本年', '累计'],
        desc: '统计周期内立案处罚的主体对象数量',
      },
      {
        id: 'rectifyOrderCount',
        label: '整改指令书下发数',
        value: '18',
        group: '执法处置',
        type: '期间',
    ...
    for (var tci = 0; tci < showTasks.length; tci++) {
      ...
      html +=
        ...
        '<circle cx="27" cy="27" r="19" fill="none" stroke="#98a2b3" stroke-width="5" stroke-dasharray="119" stroke-dashoffset="' +
        (119 - (119 * (parseInt(tc.progress) || 0)) / 100) +
        ...
        tcBar +
        '" stroke-width="5" stroke-dasharray="75" stroke-dashoffset="' +
        (75 - (75 * tcRate) / 100) +
        '" transform="rotate(-90 27 27)" stroke-linecap="round"/>' +
        '</svg>' +
        '<div style="display:flex;flex-direction:column;gap:2px">' +
        '<div style="font-size:10.5px;color:var(--text)">覆盖 <strong>' +
        tc.covered +
        '</strong> 家</div>' +
        '<div style="font-size:10.5px;color:var(--text);display:flex;gap:12px">' +
        '<span>隐患 <strong>' +
        (tc.hazards && tc.hazards !== '-' ? tc.hazards : '—') +
        '</strong> 个</span>' +
        '<span>未闭环 <strong>' +
        (tc.majorHazards && tc.majorHazards !== '-' ? tc.majorHazards : '—') +
        '</strong></span>' +
        '</div>' +
        '</div>' +
        '</div>' +
        ...
        (tc.type === '专项'
          ? '<span style="color:var(--blue);font-weight:500;flex-shrink:0;font-size:13px">专项</span>'
          : '') +
        '<span class="hc-name" style="flex:1;min-width:0">' +
        escapeHtml(tc.name) +
        '</span>' +
        '</div>' +
        '</div>' +
        '<div class="hc-time" style="margin-top:2px">' +
        tc.startDate +
        ' → ' +
        tc.endDate +
        '</div>' +
  ...
  function doFollowupAction(action, itemName) {
    var map = {
      ...
      提醒责任人: function () {
        showToast('已提醒 ' + itemName + ' 责任人');
      },
      提醒履职: function () {
        showToast('已提醒 ' + itemName + ' 责任人履职');
      },
      要求反馈: function () {
        showToast('已通知 ' + itemName + ' 需反馈进展');
      },
      要求现场核查: function () {
        showToast('已通知 ' + itemName + ' 需现场核查');
      },
      发起约谈: function () {
        openDrawer('meeting');
      },
      升级处置: function () {
        showToast('已升级处置：' + itemName);
      },
    };
    var fn = map[action];
    if (fn) fn();
    else showToast('执行动作：' + action);
  }
  ...
  function handleTodayFocusAction(actionType, itemName) {
    switch (actionType) {
      case 'supervise':
        ...
        break;
      case 'remind':
        showToast('已通知「' + itemName + '」责任人反馈');
        break;
      case 'track':
        showToast('已将「' + itemName + '」加入重点跟进');
        break;
      default:
        showToast('执行动作：' + itemName);
    }
  }
  ...
  function handleActionItemPrimary(callback, itemTitle) {
    switch (callback) {
      case 'doSupervise':
        ...
        break;
      case 'doTrack':
        showToast('已将「' + itemTitle + '」加入重点跟进');
        break;
      case 'doRequestExplain':
        showToast('已发送要求说明通知至「' + itemTitle + '」责任人');
        break;
      default:
        showToast('执行：' + itemTitle);
    }
  }
  ...
  function handleActionItemSecondary(action, itemTitle) {
    switch (action) {
      case '忽略':
        showToast('已忽略「' + itemTitle + '」');
        break;
      case '编辑':
        showToast('编辑「' + itemTitle + '」');
        break;
      case '改为督办':
        openSuperviseDrawer();
        break;
  ...
  window.openActionConfirmation = function () {
    ...
    // 默认全选
    state.selectedPAIds = {};
    for (var si = 0; si < pendingPas.length; si++) {
      state.selectedPAIds[pendingPas[si].id] = true;
    }
    ...
    var footerHtml =
      ...
      '</select>' +
      '<button onclick="executeBatchPush()" style="background:#2563eb;color:#fff;border:none;border-radius:10px;padding:7px 20px;font-size:13px;font-weight:600;cursor:pointer;transition:background .15s;display:flex;align-items:center;gap:6px" onmouseover="this.style.background=\'#1d4ed8\'" onmouseout="this.style.background=\'#2563eb\'">' +
  ...
  window.executeBatchPush = function () {
    ...
    var pas = MOCK.pendingActions || [];
    for (var ei = 0; ei < pas.length; ei++) {
      if (selected[pas[ei].id] && pas[ei].actionType === 'meeting') {
        hasMeeting = true;
        break;
      }
    }
    ...
    if (hasMeeting) {
      ...
      // 这里后续可弹出会议创建面板：日历、议程、主题、资料
    } else {
      showToast('已批量下发 ' + ids.length + ' 项至「' + (levelNames[level] || level) + '」层级');
    }
  };
  ...
  function toggleActionItemCheck(idx, checked) {
    if (checked) {
      selectedActionItemIdx[idx] = true;
    } else {
      delete selectedActionItemIdx[idx];
    }
  ...
  function updateActionItemBatchBar() {
    ...
    var items = MOCK.actionItems || [];
    var n = 0;
    for (var k in selectedActionItemIdx) {
      if (selectedActionItemIdx.hasOwnProperty(k)) n++;
    }
    ...
    for (var k = 0; k < hazards.length; k++) {
      ...
      if (h.level === '重大事故隐患' && h.status === '超期未整改') {
        ...
        for (var e = 0; e < items.length; e++) {
          if (items[e].title.indexOf(h.object) >= 0) {
            exists = true;
            break;
          }
        }
  ...
  function openSuperviseDrawer() {
    ...
    var bodyHtml =
      '<div class="drawer-supervise-intro">' +
      '<div class="dr-ai-icon"><i data-lucide="bot" width="16" height="16"></i></div>' +
      '<div style="font-size:13px;color:var(--text);line-height:1.5">基于今日全量监管上下文，系统识别出 <strong>' +
      items.length +
      ' 项</strong>需督办事项。请逐条确认后批量发起。</div>' +
      '</div>';

    for (var i = 0; i < items.length; i++) {
      var item = items[i];
      bodyHtml +=
        '<div class="drawer-supervise-item" data-si-idx="' +
        i +
        '">' +
        '<div class="dsi-head">' +
        '<span class="dsi-num">#' +
        ...
        '</h4>' +
        '</div>' +
        '<div class="dsi-section"><span class="dsi-label">督办原因</span><span class="dsi-value">' +
        item.reason +
        '</span></div>' +
        '<div class="dsi-section"><span class="dsi-label">整改要求</span><span class="dsi-value">' +
        item.requirement +
        '</span></div>' +
        '<div class="dsi-section"><span class="dsi-label">截止时间</span><span class="dsi-value">' +
        item.deadline +
        '</span></div>' +
        '<div class="dsi-section"><span class="dsi-label">升级规则</span><span class="dsi-value">' +
        item.escalation +
        '</span></div>' +
        (item.subjects.length > 0
          ? '<div class="dsi-section"><span class="dsi-label">关联主体</span><span class="dsi-value">' +
            item.subjects.join('、') +
            '</span></div>'
          : '') +
        '</div>';
  ...
  function renderPriorityItem(item) {
    ...
    return (
      ...
      item.id +
      '">' +
      '<div class="pi-left"><span class="pi-index">#' +
      item.index +
      '</span><span class="pi-tag ' +
      item.level +
      '">' +
      item.tag +
      '</span></div>' +
      '<div class="pi-body"><div class="pi-title">' +
      escapeHtml(item.title) +
      '</div>' +
      '<div class="pi-detail"><span><i data-lucide="clock" aria-hidden="true"></i>' +
      item.detail.split('·').join('</span><span><i data-lucide="user" aria-hidden="true"></i>') +
      '</span></div>' +
  ...
  function renderHazardReport() {
    ...
    for (var i = 0; i < h.length; i++) {
      rows +=
        '<tr><td>' +
        escapeHtml(h[i].object) +
        '</td><td>' +
        escapeHtml(h[i].hazard) +
        '</td><td>' +
        h[i].region +
        '</td>' +
        '<td><span class="ht-status ' +
        h[i].prevStatusCls +
        ...
        statusChange(h[i].prevStatus, h[i].status, h[i].prevStatusCls, h[i].statusCls) +
        '</div></td>' +
        '<td>' +
        h[i].person +
        '</td>' +
    ...
    for (var ri = 0; ri < h.length; ri++) {
      if (h[ri].statusCls === 'danger' || h[ri].statusCls === 'warning') {
        reviewItems +=
          '<div class="info-list-item"><div class="il-left"><i data-lucide="alert-circle" aria-hidden="true" style="color:' +
          (h[ri].statusCls === 'danger' ? 'var(--red)' : 'var(--orange)') +
          '"></i>' +
          '<span class="il-label"><strong>' +
          escapeHtml(h[ri].object) +
          '</strong> · ' +
          escapeHtml(h[ri].hazard) +
          '</span></div></div>' +
    ...
    return (
      ...
      '<div class="hazard-stat"><div class="hazard-stat-value danger">' +
      countByStatus(h, '超期未整改') +
      '</div><div class="hazard-stat-label">超期未整改</div><div class="hazard-stat-sub">较昨日 <span class="delta-up">+0</span> ⚠ 一周前也是这 2 项</div></div>' +
      '<div class="hazard-stat"><div class="hazard-stat-value warning">' +
      countByStatus(h, '即将到期') +
      '</div><div class="hazard-stat-label">即将到期</div><div class="hazard-stat-sub">3 天内到期</div></div>' +
      '<div class="hazard-stat"><div class="hazard-stat-value">' +
      countByStatus(h, '整改中') +
      '</div><div class="hazard-stat-label">整改推进中</div><div class="hazard-stat-sub"><span class="delta-down">较昨日改善 +1</span></div></div>' +
      '<div class="hazard-stat"><div class="hazard-stat-value warning">' +
      countByStatus(h, '反复出现') +
      '</div><div class="hazard-stat-label">反复出现</div><div class="hazard-stat-sub">已持续 2 周</div></div>' +
      '</div>' +
      // 隐患回头看
      '<div class="info-card">' +
      '<div class="info-card-head"><h3><i data-lucide="eye" aria-hidden="true" class="c-red"></i> 重大隐患回头看</h3><span class="info-card-badge danger">需确认 ' +
      countByStatus(h, '超期未整改') +
      '</span></div>' +
      '<div class="info-list">' +
      reviewItems +
  ...
  function countByStatus(arr, status) {
    var c = 0;
    for (var i = 0; i < arr.length; i++) {
      if (arr[i].status.indexOf(status) > -1 || status === arr[i].status) c++;
    }
    return c;
  }
  ...
  function renderEfficiency() {
    ...
    for (var gi = 0; gi < e.groups.length; gi++) {
      ...
      for (var mi = 0; mi < g.metrics.length; mi++) {
        metricsHtml +=
          '<div class="eff-metric"><span class="eff-metric-label"><i data-lucide="' +
          (iconKeys[mi] || 'chevron-right') +
          '" aria-hidden="true"></i> ' +
          g.metrics[mi].label +
          '</span><span class="eff-metric-value ' +
          g.metrics[mi].cls +
          '">' +
          g.metrics[mi].value +
          '</span></div>';
      ...
      cardsHtml +=
        '<div class="eff-card">' +
        '<div class="eff-head"><h3><i data-lucide="' +
        g.icon +
        '" aria-hidden="true"></i> ' +
        escapeHtml(g.name) +
        '</h3><span class="eff-status ' +
        g.status +
        '">' +
        (g.status === 'danger' ? '需关注' : '异常') +
        '</span></div>' +
    ...
    for (var ai = 0; ai < e.alerts.length; ai++) {
      alertsHtml +=
        '<div class="eff-alert"><i data-lucide="alert-triangle" aria-hidden="true"></i><div class="eff-alert-text"><strong>效能异常提示</strong> ' +
        e.alerts[ai] +
        ' <span class="eff-alert-action" onclick="showToast(\'已记录，将通知相关负责人核查原因\')">核查原因 →</span></div></div>';
    }
    ...
    return (
      ...
      cardsHtml +
      '<div class="eff-card full-width"><div class="eff-head"><h3><i data-lucide="alert-triangle" aria-hidden="true" class="c-orange"></i> 异常偏低提示</h3></div>' +
      alertsHtml +
      '</div>' +
      '</div>'
    );
  ...
  function renderResponsibility() {
    ...
    for (var i = 0; i < MOCK.subjects.length; i++) {
      ...
      rows +=
        '<tr><td><a href="javascript:void(0)" onclick="openEnterprisePanel(\'' +
        s.name.replace(/'/g, "\\'") +
        '\');return false" style="color:var(--blue);text-decoration:none;border-bottom:1px dashed var(--blue);cursor:pointer">' +
        escapeHtml(s.name) +
        '</a></td><td>' +
        escapeHtml(s.selfCheck) +
        '</td><td>' +
        escapeHtml(s.govCheck) +
        '</td><td>' +
        escapeHtml(s.training) +
        '</td><td>' +
        escapeHtml(s.drill) +
        '</td><td><span class="st-risk ' +
        s.risk +
        '">' +
        riskLabel +
        '</span></td><td style="font-size:12px;color:var(--accent);font-weight:500;cursor:pointer" onclick="showToast(\'已记录建议：' +
        s.suggest.replace(/'/g, "\\'") +
        '\')">' +
        escapeHtml(s.suggest) +
        '</td></tr>';
    }
    ...
    return (
      ...
      (function () {
        ...
        var sectionHtml =
          ...
          '<div style="flex:1;text-align:center;padding:12px 0;border-right:1px solid var(--line)"><div style="font-size:22px;font-weight:800;color:' +
          (selfCheckRate < 60 ? 'var(--red)' : 'var(--green)') +
          '">' +
          selfCheckRate +
          '%</div><div style="font-size:10.5px;color:var(--weak);margin-top:2px">隐患自查上报率</div><div style="font-size:9px;color:var(--muted);margin-top:1px">' +
          noSelfCheck +
          '/' +
          total +
          ' 企业未自查</div></div>' +
          '<div style="flex:1;text-align:center;padding:12px 0;border-right:1px solid var(--line)"><div style="font-size:22px;font-weight:800;color:' +
          (trainingRate < 60 ? 'var(--red)' : 'var(--green)') +
          '">' +
          trainingRate +
          '%</div><div style="font-size:10.5px;color:var(--weak);margin-top:2px">员工培训完成率</div><div style="font-size:9px;color:var(--muted);margin-top:1px">' +
          trainingLow +
          '/' +
          total +
          ' 企业不达标</div></div>' +
          '<div style="flex:1;text-align:center;padding:12px 0"><div style="font-size:22px;font-weight:800;color:' +
          (drillRate < 60 ? 'var(--red)' : 'var(--green)') +
          '">' +
          drillRate +
          '%</div><div style="font-size:10.5px;color:var(--weak);margin-top:2px">应急逃生演练完成率</div><div style="font-size:9px;color:var(--muted);margin-top:1px">' +
          noDrill +
          '/' +
          total +
          ' 企业未开展</div></div>' +
        ...
        if (gapSorted.length > 0) {
          ...
          for (var gi = 0; gi < gapSorted.length; gi++) {
            ...
            sectionHtml +=
              ...
              '<td style="padding:4px 6px;font-weight:500"><span style="display:inline-block;width:5px;height:5px;border-radius:50%;background:' +
              gRisk +
              ';margin-right:4px;vertical-align:middle"></span><a href="javascript:void(0)" onclick="openEnterprisePanel(\'' +
              g.name.replace(/'/g, "\\'") +
              '\');return false" style="color:var(--blue);text-decoration:none;border-bottom:1px dashed var(--blue);cursor:pointer">' +
              escapeHtml(g.name) +
              '</a></td>' +
              '<td style="padding:4px 6px;text-align:center;color:' +
              (g.selfCheck === 0 || g.selfCheck === '0 次' ? 'var(--red)' : 'var(--muted)') +
              '">' +
              (g.selfCheck || '0 次') +
              '</td>' +
              '<td style="padding:4px 6px;text-align:center;color:var(--muted)">' +
              g.govCheck +
              '</td>' +
              '<td style="padding:4px 6px;text-align:center;font-weight:700;color:' +
              (gapVal > 5 ? 'var(--red)' : '#d97706') +
              '">' +
              gapVal +
              '</td>' +
              '<td style="padding:4px 6px;text-align:center;color:' +
              (parseInt(g.training) < 30 ? 'var(--red)' : 'var(--muted)') +
              '">' +
              g.training +
              '</td>' +
              '<td style="padding:4px 6px;text-align:center;color:' +
              (g.drill === '0 次' || g.drill === 0 ? 'var(--red)' : 'var(--green)') +
              '">' +
              (g.drill || '0 次') +
              '</td>' +
              '</tr>';
  ...
  function renderDisposal() {
      ...
      for (var li = 0; li < arr.length; li++) {
        ...
        for (var ii = 0; ii < lv.items.length; ii++) {
          ...
          itemsHtml +=
            ...
            '<div class="dl-item-body"><div class="dl-item-title">' +
            escapeHtml(lv.items[ii].title) +
            '</div><div class="dl-item-desc">' +
            escapeHtml(lv.items[ii].desc) +
            '</div></div>' +
            '<button class="dl-item-action" onclick="' +
            (btnAction === 'remind-all' ? "openDrawer('remind')" : "showToast('动作已记录，将跟踪闭环')") +
            '"><i data-lucide="' +
            btnIcon +
            '" aria-hidden="true"></i> ' +
            btnLabel +
        ...
        h +=
          ...
          '<div class="dl-head"><span class="dl-tag ' +
          lv.tag +
          '">L' +
          lv.level +
          '</span><span class="dl-title">' +
          escapeHtml(lv.levelName) +
          '</span></div>' +
          '<div class="dl-list">' +
          itemsHtml +
          '</div></div>';
    ...
    html +=
      '<div class="section-head" style="margin-bottom:0;margin-top:32px"><h2><i data-lucide="git-branch" aria-hidden="true" class="c-accent"></i> 分级处置闭环</h2></div>';
    ...
    if (dSys.length === 0) {
      html +=
        '<div style="padding:16px 0;text-align:center;color:var(--muted);font-size:13px">暂无系统性改进建议</div>';
    } else {
      for (var si = 0; si < dSys.length; si++) {
        html +=
          '<div class="dl-item">' +
          '<div class="dl-item-icon level-5"><i data-lucide="' +
          dSys[si].icon +
          '" aria-hidden="true"></i></div>' +
          '<div class="dl-item-body"><div class="dl-item-title">' +
          escapeHtml(dSys[si].title) +
          '</div><div class="dl-item-desc">' +
          escapeHtml(dSys[si].desc) +
          '</div></div>' +
  ...
  function renderDisposalRecommendations() {
    ...
    var html =
      '<div class="section-head" style="margin-bottom:0;margin-top:24px"><h2><i data-lucide="sparkles" aria-hidden="true" class="c-accent"></i> AI 处置建议</h2><span class="info-card-badge" style="background:var(--accent);color:#fff">' +
      recs.length +
      ' 条待确认</span></div>';
    html +=
      '<div class="info-card"><div class="info-card-head"><h3><i data-lucide="bot" aria-hidden="true" class="c-accent"></i> 基于异常诊断自动生成</h3><button class="dl-item-action" onclick="regenerateDisposalRecs()" style="padding:4px 10px;font-size:11px"><i data-lucide="refresh-cw" width="12" height="12"></i> 重新生成</button></div>';
    ...
    for (var ri = 0; ri < recs.length; ri++) {
      ...
      html +=
        ...
        '<span class="dr-hazard-name">' +
        r.sourceHazard +
        '</span>' +
        '<span class="badge ' +
        riskBadgeCls +
        '" style="font-size:9px;padding:1px 6px">' +
        (r.riskLevel === 'danger' ? '危险' : '预警') +
        '</span>' +
        '</div>' +
        '</div>' +
        ...
        '<span class="dr-level-badge" style="background:' +
        levelBg +
        ';color:' +
        levelColor +
        '"><i data-lucide="' +
        getLevelDot(r.suggestedLevel) +
        '" width="11" height="11" class="v-middle"></i>' +
        r.suggestedLevelLabel +
        '</span>' +
        '</div>' +
        '<div class="dr-body-row">' +
        '<span class="dr-label">推荐动作</span>' +
        '<span class="dr-action-badge"><i data-lucide="' +
        actionIcon +
        '" width="11" height="11" class="v-middle"></i>' +
        ...
        '<span class="dr-rationale">' +
        r.rationale +
        '</span>' +
        '</div>' +
        '<div class="dr-body-row">' +
        '<span class="dr-label">处置文案</span>' +
        '<div class="dr-generated-text" id="drText_' +
        ri +
        '">' +
        r.generatedText +
        '</div>' +
        ...
        '<button class="dr-action-btn primary" onclick="copyDisposalRec(' +
        ri +
        ')" title="复制文案"><i data-lucide="copy" width="13" height="13"></i> 复制文案</button>' +
    ...
    if (dangerRules.length > 0) {
      ...
      html +=
        '<div class="dr-rule-section"><div class="dr-rule-head"><i data-lucide="settings-2" width="15" height="15" class="c-orange"></i> 规则引擎触发 · ' +
        dangerRules.length +
        ' 条危险规则匹配</div>';
      for (var dri = 0; dri < dangerRules.length; dri++) {
        ...
        var levelMap = ruleDimToLevel[ruleDim] || {
          level: 'internal-3',
          label: '内部第 3 级 · 一键提醒履职',
          action: 'remind',
        };
        var lvlHtml =
          '<span class="dr-level-badge" style="background:var(--red-soft);color:var(--red)"><i data-lucide="alert-triangle" width="11" height="11" class="v-middle"></i>' +
          levelMap.label +
          '</span>';
        html +=
          '<div class="dr-rule-item">' +
          '<div class="dr-rule-item-left"><i data-lucide="zap" width="14" height="14" class="c-red"></i></div>' +
          '<div class="dr-rule-item-body">' +
          '<div class="dr-rule-item-name">' +
          ruleName +
          '</div>' +
          '<div class="dr-rule-item-detail">' +
          (dr.detail || '') +
          '</div>' +
          '<div class="dr-rule-item-level">' +
          lvlHtml +
          '</div>' +
          '</div>' +
  ...
  YAQ.generateAllDisposalText = function () {
    ...
    try {
      document.execCommand('copy');
      showToast('已复制全部 ' + recs.length + ' 条处置建议');
    } catch (e) {
      showToast('复制失败');
    }
    document.body.removeChild(ta);
  ...
  function renderFollowup() {
    ...
    var html =
      '<div class="section-head" style="margin-bottom:0"><h2><i data-lucide="list-checks" aria-hidden="true" class="c-accent"></i> 重点跟进</h2></div>' +
      // ── 跟踪统计概览 ──────────────────────────────
      '<div style="display:flex;gap:8px;margin-bottom:12px">' +
      '<div class="info-card-badge" style="flex:1;text-align:center;background:var(--accent);color:#fff;font-size:12px;padding:6px 4px">跟踪中 ' +
      stats.tracking +
      '</div>' +
      '<div class="info-card-badge" style="flex:1;text-align:center;background:var(--orange);color:#fff;font-size:12px;padding:6px 4px">推进中 ' +
      stats.progressing +
      '</div>' +
      '<div class="info-card-badge" style="flex:1;text-align:center;background:var(--green);color:#fff;font-size:12px;padding:6px 4px">已闭环 ' +
      stats.resolved +
      '</div>' +
      '</div>' +
      '<div class="info-card">' +
      '<div class="info-card-head">' +
      '<h3>跟踪事项</h3>' +
      '<span class="info-card-badge" style="background:var(--accent);color:#fff">' +
      tracks.length +
      ' 项进行中</span>' +
      '</div>' +
    ...
    if (tracks.length === 0) {
      html +=
        '<div style="text-align:center;padding:24px 0;color:var(--muted);font-size:13px">暂无跟踪事项。<br>在诊断或处置页面点击「加入跟踪」即可开始跟踪。</div>';
    } else {
      for (var fui = 0; fui < tracks.length; fui++) {
        ...
        html +=
          ...
          escapeHtml(fu.title) +
          '</span>' +
          '<span style="font-size:10px;padding:1px 6px;border-radius:3px;white-space:nowrap;background:' +
          ...
          '</div>' +
          '<span style="font-size:10px;color:var(--weak);min-width:32px;text-align:right">' +
          fu.progress +
          '%</span>' +
          '</div>' +
          // 元信息
          '<div style="font-size:11px;color:var(--muted);line-height:1.5">' +
          '<span class="c-weak">跟踪天数：</span>' +
          (fu.daysTracked || 0) +
          ' 天' +
          ...
          (fu.updates && fu.updates.length > 0
            ? '<div style="font-size:11px;color:var(--muted);line-height:1.5;background:var(--bg);padding:6px 8px;border-radius:4px">' +
              '<span class="c-weak">最新：</span>' +
              escapeHtml(fu.updates[fu.updates.length - 1].text) +
              '</div>'
            : '') +
          // 时间线（最近2条）
          ...
          (fu.status === 'tracking'
            ? '<button onclick="event.stopPropagation();YAQ.updateTrackProgress(\'' +
              fu.id +
              '\')" style="font-size:10px;padding:3px 8px;border-radius:4px;cursor:pointer;background:var(--accent);color:#fff;border:none">更新进展</button>' +
              '<button onclick="event.stopPropagation();YAQ.resolveTrack(\'' +
              fu.id +
              '\')" style="font-size:10px;padding:3px 8px;border-radius:4px;cursor:pointer;background:var(--green);color:#fff;border:none">标记闭环</button>'
            : fu.status === 'progressing'
              ? '<button onclick="event.stopPropagation();YAQ.updateTrackProgress(\'' +
                ...
                '<button onclick="event.stopPropagation();YAQ.resolveTrack(\'' +
                fu.id +
                '\')" style="font-size:10px;padding:3px 8px;border-radius:4px;cursor:pointer;background:var(--green);color:#fff;border:none">确认闭环</button>'
              : fu.status === 'resolved'
                ? '<button onclick="event.stopPropagation();YAQ.closeTrack(\'' +
                  fu.id +
                  '\')" style="font-size:10px;padding:3px 8px;border-radius:4px;cursor:pointer;background:var(--weak);color:#fff;border:none">归档</button>'
                : '') +
          (fu.needIntervention
            ? '<span style="font-size:10px;color:var(--red);font-weight:600;margin-left:auto">⚠ 需介入</span>'
            : '') +
          '</div>' +
          '</div>';
      }
  ...
  var drawerContent = {
      ...
      sections: [
        {
          label: '今日判断',
          value:
            '总体平稳可控，但重大隐患闭环压力上升。今日需优先处理 2 项已超期重大隐患，随后核查高层小区专项任务滞后原因，并关注 8 家主体对象自查与政府检查不匹配。',
        },
        {
          label: '主要依据',
          value:
            '新增隐患 12 项（较日均+20%）；村社履职率 76%（目标 80%）；主体责任异常 8 家；重大未闭环 5 项，其中超期 2 项。',
        },
        {
          label: '优先动作',
          value:
            '1. 确认北苑商业综合体、云栖高层住宅整改方案\n2. 核查高层小区消防专项滞后原因\n3. 安排杭州恒源化工有限公司、杭州鑫盛机械制造有限公司主体责任约谈',
        },
        {
          label: '数据限制',
          value:
            '履职率口径尚未统一定义；风险预测数据不足 6 个月，暂不适合强预测。以上判断基于当前可用数据，建议结合实际情况综合研判。',
        },
      ],
    },
    supervise: {
    ...
    meeting: {
      ...
      sections: [
        { label: '议题标题', value: '近期重大隐患整改推进及主体责任约谈计划' },
        {
          label: '问题背景',
          value:
            '当前重大未闭环 5 项，其中 2 项已超期；8 家主体对象自查为 0 但政府检查发现多项隐患，存在敷衍自查迹象。',
        },
        {
          label: '需会议确认',
          value: '1. 北苑商业综合体、云栖高层住宅整改方案\n2. 高层小区消防专项推进计划\n3. 主体责任约谈名单及时间安排',
        },
        {
          label: '建议决策',
          value:
    ...
    remind: {
      ...
      sections: [
        ...
        {
          label: '提醒事由',
          value: '余杭东兴精密机械厂自查缺失+培训不足、云栖高层住宅消防设施失效，需相关责任人员现场核查隐患整改进展。',
        },
        {
          label: '提醒方式',
          value:
            '系统自动发送浙政钉通知 + 短信提醒至以下角色：\n• 应消站值班人员\n• 区域站负责人\n• 第三方安全专家\n• 属地村社负责人',
        },
  ...
  function renderPendingActions() {
    ...
    html +=
      ...
      '<span class="info-card-badge" style="background:var(--orange);color:#fff">' +
      pendingPas.length +
      ' 项待确认</span>' +
      '</div>';
    ...
    // ─── 批量操作栏 ──────────────────────────────────────────
    html +=
      '<div class="pa-batch-bar" id="paBatchBar">' +
    ...
    if (pendingPas.length === 0) {
      ...
      html +=
        '<div class="pa-section-label"><i data-lucide="clock" width="15" height="15"></i> 待确认行动 <span class="pa-section-hint">（点击卡片左侧复选框可多选批量操作）</span></div>';
      for (var pi = 0; pi < pendingPas.length; pi++) {
    ...
    if (confirmed.length > 0) {
      ...
      html +=
        '<div class="pa-section-label" style="margin-top:4px"><i data-lucide="check-circle" width="15" height="15"></i> 已确认的督办包</div>';
    ...
    if (donePas.length > 0) {
      html += '<div class="pa-section-divider"></div>';
      html +=
        '<div class="pa-section-label" style="margin-top:4px"><i data-lucide="archive" width="15" height="15"></i> 其他处理记录</div>';
      for (var di = 0; di < donePas.length; di++) {
        var d = donePas[di];
        var dLabel =
          d.actionType === 'track'
            ? '已加入跟进'
            : d.actionType === 'explain'
              ? '已改为要求说明'
              : d.actionType === 'observe'
                ? '暂不处理'
                : '已忽略';
        html +=
          '<div class="pa-done-card">' +
          '<div style="display:flex;align-items:center;gap:8px">' +
          '<span style="font-size:11px;color:var(--weak)">' +
          dLabel +
          '</span>' +
          '<span style="font-size:13px;font-weight:600;color:var(--muted);text-decoration:line-through">' +
          d.title +
          '</span>' +
          '</div>' +
          '</div>';
  ...
  function renderPendingActionCard(pa) {
    ...
    var actionConfig = {
      ...
      meeting: { label: '建议开会讨论', cls: 'pa-type-supervise', icon: 'message-square' },
    };
    ...
    for (var key in pa.chain) {
      if (pa.chain.hasOwnProperty(key)) {
        ...
        chainHtml +=
          '<span class="pa-chain-badge"><span class="pa-chain-role">' +
          role.label +
          '</span>' +
          role.person +
          '</span>';
    ...
    // 来源类型标签
    var sourceHtml = '';
    if (pa.mergedFrom && pa.mergedFrom.length > 0) {
      var types = {};
      var events = MOCK.abnormalEvents || [];
      for (var ei = 0; ei < events.length; ei++) {
        if (pa.mergedFrom.indexOf(events[ei].id) > -1) {
    ...
    for (var ii = 0; ii < pa.draftItems.length; ii++) {
      ...
      itemsHtml +=
        '<div class="pa-draft-item">' +
        '<div class="pa-di-left">' +
        '<span class="pa-di-role">' +
        di.role +
        '</span>' +
        '<span class="pa-di-task">' +
        di.task +
        '</span>' +
        '</div>' +
        '<span class="pa-di-deadline">' +
        di.deadline +
        '</span>' +
        '</div>';
    }
    ...
    var html =
      ...
      '<span class="pa-type-tag"><i data-lucide="' +
      cfg.icon +
      ...
      '</div>' +
      // 依据
      '<div class="pa-card-basis"><i data-lucide="file-text" width="13" height="13"></i> ' +
      pa.basis +
      '</div>' +
      // 统计
      '<div class="pa-card-stats">' +
      '<span class="pa-stat"><i data-lucide="alert-circle" width="13" height="13"></i> 关联异常 ' +
      mergedCount +
      ' 条</span>' +
      (subjectCount > 0
        ? '<span class="pa-stat"><i data-lucide="building-2" width="13" height="13"></i> 关联主体 ' +
          subjectCount +
          ' 家</span>'
        : '') +
      sourceHtml +
      ...
      (pa.affectedSubjects && pa.affectedSubjects.length > 0
        ? '<div class="pa-card-subjects"><span class="pa-sub-label">关联主体：</span>' +
          pa.affectedSubjects.join('、') +
          '</div>'
        : '') +
      // 责任链
      '<div class="pa-card-chain">' +
      '<div class="pa-chain-label">责任链</div>' +
      '<div class="pa-chain-badges">' +
      chainHtml +
      '</div>' +
      '</div>' +
      // 预生成处理项（可展开）
      '<div class="pa-card-items">' +
      '<div class="pa-items-label">预生成处理项 <span class="pa-items-count">' +
      pa.draftItems.length +
      ' 项</span></div>' +
      '<div class="pa-items-list">' +
      itemsHtml +
      '</div>' +
      '</div>' +
      // 操作类型切换
      ...
      pa.id +
      '\')" style="flex:1;min-width:60px;justify-content:center"><i data-lucide="x" width="13" height="13"></i> 忽略</button>' +
      '</div>' +
      '</div>' +
      '</div>';
  ...
  function renderConfirmedPackageCard(sp) {
    var statusMap = {
      推进中: { cls: 'orange', icon: 'loader' },
      已完成: { cls: 'green', icon: 'check-circle' },
      需升级: { cls: 'red', icon: 'alert-triangle' },
    };
        ...
        di.status === 'done'
          ...
          : di.status === 'in_progress'
            ...
            : di.status === 'overdue'
              ...
              : 'circle';
      itemsHtml +=
        '<div class="pa-cfi-item ' +
        diCls +
        '">' +
        '<i data-lucide="' +
        diIcon +
        '" width="13" height="13"></i>' +
        '<span class="pa-cfi-role">' +
        di.role +
        '</span>' +
        '<span class="pa-cfi-task">' +
        di.task +
        '</span>' +
        '<span class="pa-cfi-deadline">' +
        di.deadline +
        '</span>' +
        '</div>';
    }
    ...
    var html =
      ...
      sp.itemCount +
      ' 项</span>' +
      '<span class="pa-stat-done">已反馈 ' +
      sp.doneCount +
      '</span>' +
      '<span class="pa-stat-overdue">逾期 ' +
      sp.overdueCount +
      '</span>' +
      '</div>' +
      '<div class="pa-cc-items">' +
      itemsHtml +
      '</div>' +
      '</div>';
  ...
  function renderSupervisionTrack() {
    ...
    if (packages.length === 0) {
      html +=
        '<div style="padding:40px 0;text-align:center;color:var(--weak);font-size:13px">暂无已发起的督办包<br>请在「待确认行动」中确认发起督办</div>';
      return html;
    }
    ...
    sections: [
      {
        ...
        title: '一、总体情况',
        content:
          '本月，良渚街道应急消防管理站共纳入监管的责任主体14491家，其中企业951家、各类场所13540家，场所类责任主体占总数的93.44%，场所类责任主体仍是当前安全监管的重点与难点。',
        details: [
      ...
      },
      {
        ...
        analysis:
          '信息采集分析：企业采集完成情况良好，采集率达93.48%；场所采集率57.36%，明显偏低。环比分别提升2.16%和23.04%，整体采集率稳步上升。',
      },
      {
        id: 'supervision',
        ...
        analysis:
          '监管执行分析：企业待办推送覆盖率（58.57%）显著高于场所（17.05%），但整改完成率（11.13%）远低于场所（59.46%）。环比显示，场所两端指标增速均明显快于企业。',
      },
      {
        id: 'hazard',
        icon: 'alert-triangle',
      ...
      },
      {
        ...
        analysis:
          '突出问题：风险点识别（43.27%）与隐患整改优化到位（26.64%）占比较低，且环比增速分别仅为9.31%和10.94%，需强化动态风险辨识与闭环整改机制。',
      },
      {
        id: 'village',
        icon: 'map-pin',
      ...
      },
      {
        ...
        title: '三、街道内部火灾事故情况',
        content:
          '良渚街道对照：近期火情以杂草起火、锅烧焦和杂物起火为主，三者合计占比约19.2%。与往年相比，室外可燃物（如杂草、垃圾）起火频次上升，厨房用火及电气类火情仍属常见类型。当前正值干燥季节，需重点防范野外及房前屋后可燃物自燃或引燃风险。',
        details: [
          '季节性规律：火情呈季节性集中，秋冬季（10月至次年2月）发生频次较高。主要诱因类型为杂草起火、锅烧焦及杂物起火，合计占比约19.5%。',
        ],
      },
      {
        id: 'risk-predict',
  ...
  function renderMrSidebar() {
    ...
    for (var hi = 0; hi < MR_HISTORY.length; hi++) {
      var h = MR_HISTORY[hi];
      html +=
        '<div onclick="YAQ.switchMrHistory(\'' +
        h.id +
        '\')" style="padding:8px 10px;border-radius:8px;cursor:pointer;font-size:12px;margin-bottom:4px;border:1px solid ' +
        (h.active ? 'var(--accent)' : 'transparent') +
        ';background:' +
        (h.active ? '#eef2ff' : 'transparent') +
        ';transition:all .15s" onmouseenter="this.style.background=\'#f2f4f7\'" onmouseleave="this.style.background=\'' +
        (h.active ? '#eef2ff' : 'transparent') +
        '\'">' +
        ...
        h.label +
        '</div>' +
        '<div style="font-size:10px;color:var(--weak);margin-top:2px">' +
        h.period +
        '</div>' +
  ...
  function renderMonthlyReport() {
    ...
    html +=
      ...
      '<h2 style="margin:0;font-size:18px;font-weight:700;color:var(--text)"><i data-lucide="calendar" aria-hidden="true" style="color:var(--accent);margin-right:6px"></i> ' +
      d.title +
      '</h2>' +
      '<span style="font-size:11px;color:var(--weak);background:var(--bg);padding:2px 10px;border-radius:20px;border:1px solid var(--line)">' +
      d.period +
      '</span>' +
      '</div>' +
      '<div style="font-size:12px;color:var(--weak);margin-top:2px">' +
      d.org +
      ' · ' +
      d.date +
      '</div>' +
    ...
    html +=
      ...
      d.summary.totalSubjects.toLocaleString() +
      '</div>' +
      '<div style="font-size:10px;color:var(--weak);margin-top:2px">企业 ' +
      d.summary.enterprises +
      ' · 场所 ' +
      d.summary.venues.toLocaleString() +
      '</div>' +
      '</div>' +
      ...
      d.summary.collectionRate +
      '</div>' +
      '<div style="font-size:10px;color:#64748b;margin-top:2px">企业 ' +
      d.summary.enterpriseCollection +
      ' · 场所 ' +
      d.summary.venueCollection +
      '</div>' +
      '</div>' +
    ...
    for (var si = 0; si < d.sections.length; si++) {
      ...
      html +=
        '<div class="mr-section" style="margin-bottom:12px;border:1px solid var(--line);border-radius:12px;background:var(--card);overflow:hidden">' +
        ...
        '<i data-lucide="' +
        sec.icon +
        '" width="16" height="16" style="color:var(--accent);flex-shrink:0"></i>' +
        '<span style="font-size:13px;font-weight:600;color:var(--text);flex:1">' +
        sec.title +
        '</span>' +
      ...
      if (sec.details) {
        for (var di = 0; di < sec.details.length; di++) {
          html +=
            '<p style="margin:4px 0;color:var(--weak);font-size:12.5px;padding-left:8px;border-left:2px solid var(--line)">' +
            sec.details[di] +
            '</p>';
        }
      }
      ...
      if (sec.table) {
        ...
        for (var hi2 = 0; hi2 < sec.table.headers.length; hi2++) {
          html +=
            '<th style="padding:7px 10px;text-align:left;font-weight:600;color:var(--text);border-bottom:1px solid var(--line);white-space:nowrap">' +
            sec.table.headers[hi2] +
            '</th>';
        }
        html += '</tr></thead><tbody>';
        for (var ri = 0; ri < sec.table.rows.length; ri++) {
          ...
          html +=
            '<tr' + (ri === sec.table.rows.length - 1 ? '' : ' style="border-bottom:1px solid var(--line)"') + '>';
  ...
  YAQ.mrToggleModule = function (sectionId) {
    ...
    if (btn) {
      ...
      for (var i = 0; i < MONTHLY_REPORT_DATA.sections.length; i++) {
        if (MONTHLY_REPORT_DATA.sections[i].id === sectionId) {
          sec = MONTHLY_REPORT_DATA.sections[i];
          break;
        }
      }
  ...
  YAQ.mrAddModule = function () {
    ...
    setTimeout(function () {
      ...
      var exists = false;
      for (var i = 0; i < MONTHLY_REPORT_DATA.sections.length; i++) {
        if (MONTHLY_REPORT_DATA.sections[i].id === 'yoy-analysis') {
          exists = true;
          break;
        }
      }
      if (!exists) {
        MONTHLY_REPORT_DATA.sections.push({
          ...
          analysis:
            'AI自动生成的同比分析：与去年同期相比，监管覆盖面大幅提升，各项指标从无到有建立。建议持续关注场所采集率提升空间。',
        });
  ...
  function openDrawer(action) {
    ...
    $dom.drawerTitle.innerHTML =
      '<i data-lucide="' + getDrawerIcon(action) + '" aria-hidden="true"></i> ' + content.title;

    var bodyHtml = '';
    for (var i = 0; i < content.sections.length; i++) {
      var sec = content.sections[i];
      bodyHtml +=
        '<div class="drawer-section"><div class="drawer-section-label">' +
        sec.label +
        '</div><div class="drawer-section-value">' +
        sec.value.replace(/\n/g, '<br>') +
        '</div></div>';
  ...
  function openAgentConfig(sceneId) {
    ...
    for (var p = 0; p < agentSchedulePresets.length; p++) {
      ...
      promptOptionsHtml +=
        '<option value="' + escapeHtml(preset.cron) + '"' + selected + '>' + preset.label + '</option>';
    }

    var bodyHtml =
      '<div class="agent-config-section">' +
      '<label><i data-lucide="message-square" width="13" height="13"></i> 提示词</label>' +
      '<textarea class="agent-prompt-ta" id="agentPromptEditable" placeholder="在此编辑提示词…">' +
      escapeHtml(savedPrompt || defaultPrompt) +
      '</textarea>' +
      '<div class="agent-config-hint">编辑后点击保存；清空内容并保存可恢复默认提示词</div>' +
      '</div>' +
      '<div class="agent-config-section">' +
      '<label><i data-lucide="clock" width="13" height="13"></i> 执行周期</label>' +
      '<select id="agentScheduleSelect" class="agent-schedule-select">' +
      promptOptionsHtml +
      '</select>' +
      '<div class="agent-config-hint">Cron: <code id="agentCronPreview">' +
      escapeHtml(cron) +
      '</code></div>' +
      '</div>' +
      '<button class="agent-config-save" onclick="saveAgentPrompt(\'' +
      sceneId +
  ...
  function openHazardDetail(objectName, foundDate) {
    ...
    $dom.hazardModalName.innerHTML =
      '<a href="#" onclick="openEnterprisePanel(\'' +
      h.object.replace(/'/g, "\\'") +
      '\');return false" style="color:var(--text);text-decoration:none;border-bottom:1px dashed var(--blue)">' +
      escapeHtml(h.object) +
      '</a>';
    ...
    // — 顶部状态区 —
    var statusBadge =
      '<span class="hc-status ' + h.statusCls + '" style="font-size:11px;padding:2px 8px">' + h.status + '</span>';
    var overdueHtml =
      h.overdue > 0
        ? '<span style="color:var(--red);font-weight:700;font-size:13px">⚠ 逾期 ' + h.overdue + ' 天</span>'
        : '';
    var bodyHtml =
      '<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:12px">' +
      '<div style="display:flex;align-items:center;gap:8px">' +
      '<span style="font-size:10px;font-weight:600;padding:2px 8px;border-radius:999px;background:color-mix(in oklch, var(--red) 12%, #fff);color:var(--red)">' +
      h.level +
      '</span>' +
      statusBadge +
      '</div>' +
      ...
      '</div>' +
      // — 隐患描述（突出）—
      '<div style="font-size:14px;font-weight:600;color:var(--text);line-height:1.5;margin-bottom:12px;padding:10px 12px;background:var(--fg-soft);border-radius:10px">' +
      h.hazard.replace(/\n/g, '<br>') +
      '</div>' +
      // — 整改建议（突出）—
      '<div style="font-size:10px;font-weight:600;color:var(--weak);margin-bottom:4px;letter-spacing:0.05em">整改建议</div>' +
      '<div style="font-size:13px;color:#344054;line-height:1.6;margin-bottom:14px;padding:8px 12px;background:#f8fafc;border:1px solid #e2e8f0;border-radius:10px">' +
      (h.suggestion || '—') +
      '</div>' +
      // — 基础信息（两列简洁）—
      '<div style="display:grid;grid-template-columns:1fr 1fr;gap:3px 16px;margin-bottom:10px">' +
      '<div style="font-size:11.5px;color:var(--muted);padding:2px 0"><span class="c-weak">对象</span> <a href="#" onclick="openEnterprisePanel(\'' +
      h.object.replace(/'/g, "\\'") +
      '\');return false" style="color:var(--blue);text-decoration:none;border-bottom:1px dashed var(--blue)">' +
      escapeHtml(h.object) +
      '</a></div>' +
      '<div style="font-size:11.5px;color:var(--muted);padding:2px 0"><span class="c-weak">责任人</span> ' +
      (h.person || '—') +
      '</div>' +
      '<div style="font-size:11.5px;color:var(--muted);padding:2px 0"><span class="c-weak">发现</span> ' +
      (h.discoverer || '—') +
      ' / ' +
      h.foundDate +
      '</div>' +
      '<div style="font-size:11.5px;color:var(--muted);padding:2px 0"><span class="c-weak">期限</span> ' +
      h.deadline +
      '</div>' +
      '<div style="font-size:11.5px;color:var(--muted);padding:2px 0"><span class="c-weak">来源</span> ' +
      h.source +
      '</div>' +
      '<div style="font-size:11.5px;color:var(--muted);padding:2px 0"><span class="c-weak">片区</span> ' +
      (h.region || '—') +
      '</div>' +
      '</div>' +
      // — 依据（可折叠）—
      ...
      '</div>' +
      '<div id="regulationBody" style="display:none;margin-top:4px;font-size:11.5px;color:var(--muted);padding:6px 10px;background:#f9fafb;border-radius:8px">' +
      (h.regulation || '无') +
      '</div>' +
      '</div>';
    ...
    bodyHtml +=
      '<div style="font-size:10px;font-weight:600;color:var(--weak);margin-bottom:8px;letter-spacing:0.05em">现场证据</div>';
    // 问题快照
    var photoCount = h.hasPhoto ? 3 : 0;
    bodyHtml +=
      '<div class="hmodal-row" style="align-items:flex-start;padding-top:10px"><div class="hmodal-label">问题快照</div><div class="hmodal-value"><div style="display:flex;gap:8px;flex-wrap:wrap">';
    if (photoCount > 0) {
      for (var pi = 1; pi <= photoCount; pi++) {
        bodyHtml +=
          '<div style="width:80px;height:60px;border-radius:8px;background:#f2f4f7;border:1px solid var(--line);display:flex;align-items:center;justify-content:center;font-size:10px;color:var(--weak)"><i data-lucide="image" width="16" height="16" style="opacity:.4"></i></div>';
      }
    } else {
    ...
    // 整改结果
    bodyHtml +=
      '<div class="hmodal-row" style="align-items:flex-start;padding-top:10px"><div class="hmodal-label">整改结果</div><div class="hmodal-value">' +
      '<div style="display:flex;gap:8px;flex-wrap:wrap;margin-bottom:6px">' +
      (h.resultPhoto
        ? '<div style="width:80px;height:60px;border-radius:8px;background:#e8f5e9;border:1px solid #c8e6c9;display:flex;align-items:center;justify-content:center;font-size:10px;color:#2e7d32"><i data-lucide="check-circle" width="16" height="16"></i></div>'
        : '') +
      '</div>' +
    ...
    if (entData) {
      aiHtml =
        '<div style="font-size:12px;line-height:1.65;color:#344054">' + generateHazardAnalysis(h, entData) + '</div>';
    }

    $dom.hazardModalBody.innerHTML =
      '<div class="hmodal-main">' + bodyHtml + '</div>' + '<div class="hmodal-ai">' + aiHtml + '</div>';
  ...
  function generateHazardAnalysis(h, entData) {
    var allHaz = MOCK.hazards.filter(function (x) {
      return x.object === h.object;
    });
    var prevSame = allHaz.filter(function (x) {
      return x.hazard.indexOf(h.hazard.slice(0, 6)) > -1 && x !== h;
    });
    ...
    var siRate =
      si.length > 0
        ? Math.round(
            (si.filter(function (x) {
              return x.statusCls === 'done' || x.status === '无异常';
            }).length /
              si.length) *
              100,
          )
    ...
    var closedHaz = allHaz.filter(function (x) {
      return x.statusCls === 'done';
    }).length;
    ...
    var html =
      ...
      // 逾期标签
      (overdueDays > 0
        ? '<div style="background:#fef2f2;color:#dc2626;font-size:10px;font-weight:700;padding:2px 7px;border-radius:5px;display:inline-block;margin-bottom:8px">⚠ 逾期 ' +
          overdueDays +
          ' 天</div>'
        : '') +
      // 政府端
  ...
  function openEnterprisePanel(name) {
    ...
    if (!data) {
      ...
      for (var si = 0; si < MOCK.subjects.length; si++) {
        if (MOCK.subjects[si].name === name) {
          subj = MOCK.subjects[si];
          break;
        }
      }
      if (subj) {
          ...
          dimensions: [
            {
              ...
              icon: 'shield',
              text: '待完善。',
              bar: subj.risk === 'high' ? 'c' : subj.risk === 'mid' ? 'b' : 'a',
            },
            {
              id: 'inspection',
              label: '隐患排查治理',
              score: subj.risk === 'high' ? 'D' : subj.risk === 'mid' ? 'C' : 'B',
              icon: 'search',
              text: '待完善。',
              bar: 'c',
            },
            {
              id: 'training',
              label: '教育培训',
              score: subj.risk === 'high' ? 'D' : 'C',
              icon: 'graduation-cap',
              text: '培训完成率 ' + (subj.training || '—') + '。',
              bar: 'c',
            },
            {
              id: 'emergency',
              ...
              score: 'C',
              icon: 'alert-triangle',
              text: '应急演练 ' + (subj.drill || '0 次') + '。',
              bar: 'c',
            },
            {
              id: 'history',
              label: '历史表现评价',
              score: subj.risk === 'high' ? 'C' : 'B',
              icon: 'clock',
              text: '建议完善企业安全管理档案。',
              bar: 'b',
            },
  ...
  function epSwitchTab(tab) {
    ...
    cards.forEach(function (c) {
      if (c.getAttribute('data-tab') === tab) c.classList.add('mc-active');
    });
    // 更新内容区
  ...
  function epRenderFixedTop(data) {
    ...
    var unclosedHaz = data.hazards.filter(function (x) {
      return x.statusCls !== 'done';
    }).length;
    ...
    var html =
      ...
      '<span style="font-size:10px;font-weight:700;padding:2px 8px;border-radius:999px;background:' +
      rBg +
      ';color:' +
      rColor +
      ';border:1px solid ' +
      ...
      data.region +
      '</div>' +
      '<div style="font-size:11px;color:var(--muted);margin-top:1px">责任人 ' +
      (data.person || '—') +
      ' · ' +
      (data.area || '—') +
      '</div>' +
      '</div>' +
      '</div>' +
      '<div style="font-size:11px;color:var(--muted);line-height:1.5;margin-bottom:10px;padding:6px 10px;background:#fafbfc;border-radius:8px">' +
      data.summary +
      '</div>' +
      // 指标卡 = Tab
      '<div class="metric-row" style="margin-bottom:6px">' +
      '<div class="metric-card ep-tab-card' +
      (unclosedHaz > 0 ? ' alert-danger' : '') +
      (activeTab === 'hazards' ? ' mc-active' : '') +
      '" style="cursor:pointer;position:relative" data-tab="hazards" onclick="epSwitchTab(\'hazards\')">' +
      (unclosedHaz > 0 ? '<span class="mc-alert-badge">' + unclosedHaz + '</span>' : '') +
      '<div class="mc-value">' +
      data.hazards.length +
      '</div><div class="mc-label">隐患数</div></div>' +
      '<div class="metric-card ep-tab-card' +
      (activeTab === 'expert' ? ' mc-active' : '') +
      '" class="pointer" data-tab="expert" onclick="epSwitchTab(\'expert\')"><div class="mc-value">' +
      (data.expertRecords || []).length +
      '</div><div class="mc-label">专家履职</div></div>' +
      '<div class="metric-card ep-tab-card' +
      (activeTab === 'selfinspect' ? ' mc-active' : '') +
      '" class="pointer" data-tab="selfinspect" onclick="epSwitchTab(\'selfinspect\')"><div class="mc-value">' +
      (data.selfInspections || []).length +
      '</div><div class="mc-label">自检自查</div></div>' +
      '<div class="metric-card ep-tab-card' +
      (activeTab === 'training' ? ' mc-active' : '') +
      '" class="pointer" data-tab="training" onclick="epSwitchTab(\'training\')"><div class="mc-value">' +
      (data.trainingRecords || []).length +
      '</div><div class="mc-label">教育培训</div></div>' +
      '<div class="metric-card ep-tab-card' +
      (activeTab === 'dimensions' ? ' mc-active' : '') +
      '" class="pointer" data-tab="dimensions" onclick="epSwitchTab(\'dimensions\')"><div class="mc-value">' +
      (data.scorePct || '—') +
      '%</div><div class="mc-label">主体责任</div></div>' +
      '</div>';
    return html;
  }
  ...
  function epRenderTab(data, tab) {
    ...
    if (tab === 'selfinspect') {
      return epRenderList(data.selfInspections, '自检自查记录', function (s) {
        ...
        var issueText =
          s.issues > 0
            ? '<span style="color:' +
              (s.statusCls === 'danger' ? 'var(--red)' : '#d97706') +
              ';font-weight:600">问题 ' +
              s.issues +
              ' 项</span>'
            : '<span style="color:var(--green);font-weight:600">无异常</span>';
        return (
          '<div class="ep-hist-item">' +
          '<span class="ep-hist-dot ' +
          dotCls +
          '"></span>' +
          ...
          '<div style="font-size:11px;color:var(--weak)">' +
          s.date +
          '</div>' +
    ...
    if (tab === 'expert') {
      return epRenderList(data.expertRecords, '专家履职记录', function (e) {
        ...
        return (
          ...
          e.date +
          '</div>' +
          '<div style="font-size:10px;margin-top:2px;color:var(--weak)">' +
          e.org +
          '</div></div></div>'
        );
      });
    ...
    if (tab === 'training') {
      return epRenderList(data.trainingRecords, '培训记录', function (t) {
        return (
          ...
          t.date +
          '</div>' +
          '<div style="font-size:10.5px;margin-top:2px;color:var(--green)">' +
          t.attendees +
          ' 人</div></div></div>'
        );
      });
    ...
    if (tab === 'dimensions') {
      ...
      for (var di = 0; di < data.dimensions.length; di++) {
        ...
        html +=
          '<div class="ep-dim-card"><div class="ep-dim-top"><div class="ep-dim-label"><i data-lucide="' +
          dim.icon +
          '" width="14" height="14"></i>' +
          dim.label +
          '</div><span class="ep-dim-badge ' +
          dim.score.toLowerCase() +
          '">' +
          dim.score +
          '</span></div><div class="ep-dim-body">' +
          dim.text +
          '</div><div class="ep-dim-bar"><i class="' +
          dim.bar +
          '"></i></div></div>';
      }
      return html;
  ...
  function epRenderHazardCards(hazards) {
    var html =
      '<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:10px">' +
      '<div style="font-size:14px;font-weight:700">隐患记录（' +
      hazards.length +
      '）</div>' +
      '<button onclick="epSwitchTab(\'overview\')" style="width:24px;height:24px;border:none;border-radius:6px;background:transparent;color:var(--weak);cursor:pointer;display:grid;place-items:center" onmouseover="this.style.background=\'#f2f4f7\'" onmouseout="this.style.background=\'transparent\'"><i data-lucide="x" width="14" height="14"></i></button>' +
      '</div>';
    for (var i = 0; i < hazards.length; i++) {
      var h = hazards[i];
      html +=
        '<div class="hazard-card" style="cursor:pointer;margin-bottom:8px" onclick="openHazardDetail(\'' +
        h.object.replace(/'/g, "\\'") +
        "','" +
        h.foundDate +
        '\')">' +
        '<div class="hc-head"><span class="hc-name">' +
        escapeHtml(h.object) +
        '</span></div>' +
        '<div class="hc-desc">' +
        escapeHtml(h.hazard) +
        ...
        (h.status === '已完成'
          ? '<button class="hc-btn" onclick="event.stopPropagation();showToast(\'复查记录已提交\')"><i data-lucide="check-circle" width="11" height="11"></i> 复查确认</button>' +
            '<button class="hc-btn" onclick="event.stopPropagation();openHazardDetail(\'' +
            h.object.replace(/'/g, "\\'") +
            "','" +
            h.foundDate +
            '\')"><i data-lucide="file-text" width="11" height="11"></i> 查看详情</button>'
  ...
  function epRenderRecentHazards(hazards) {
    ...
    for (var i = 0; i < recent.length; i++) {
      ...
      var overdueLabel =
        h.overdue > 0
          ? '<span style="color:var(--red);font-weight:600">逾期 ' + escapeHtml(h.overdue) + ' 天</span>'
          : '<span class="c-weak">—</span>';
      html +=
        '<div class="hazard-card" style="cursor:pointer;margin-bottom:8px" onclick="openHazardDetail(\'' +
        h.object.replace(/'/g, "\\'") +
        "','" +
    ...
    if (hazards.length > 3) {
      html +=
        '<div style="text-align:center;margin-top:4px;font-size:11px;color:var(--weak)">共 ' +
        hazards.length +
        ' 条隐患，点击上方「隐患数」查看全部</div>';
    }
    return html;
  }
  ...
  function openMetricDrilldown(el) {
    ...
    if (allLines.length > 1) {
      listHtml +=
        '<div class="drill-filter" id="drillFilter">' + '<span class="df-btn active" data-line="all">全部</span>';
    ...
    for (var ii = 0; ii < items.length; ii++) {
      ...
      listHtml +=
        '<div class="drill-item" data-line="' +
        (it.line || '其他') +
        '" onclick="openHazardDetail(\'' +
        it.name.replace(/'/g, "\\'") +
        '\')" title="点击查看详情" class="pointer">' +
        '<div class="drill-item-head">' +
        '<span class="drill-item-title">' +
        it.name +
        '</span>' +
        (it.line ? '<span class="drill-item-line">' + it.line + '</span>' : '') +
        '<span class="drill-item-badge ' +
        statusCls +
        '">' +
        ...
        overdueHtml +
        '</div>' +
        '<div class="drill-item-desc">' +
        it.detail +
        '</div>' +
        '<div class="drill-item-meta">' +
        (it.person
          ? '<span class="dim-meta"><i data-lucide="user" width="10" height="10" aria-hidden="true"></i>' +
            it.person +
            '</span>'
          : '') +
        (it.source
          ? '<span class="dim-meta"><i data-lucide="clipboard-check" width="10" height="10" aria-hidden="true"></i>' +
            it.source +
            '</span>'
          : '') +
        (it.region
          ? '<span class="dim-meta"><i data-lucide="map-pin" width="10" height="10" aria-hidden="true"></i>' +
            it.region +
            '</span>'
          : '') +
        '</div>' +
    ...
    if (aiItems && aiItems.length > 0) {
      for (var ai = 0; ai < aiItems.length; ai++) {
        ...
        initialMsgHtml +=
          '<div class="drill-analysis">' +
          '<span class="drill-analysis-label ' +
          cls +
          '">' +
          a.label +
          '</span>' +
          '<div class="drill-analysis-text">' +
          a.text.replace(/\n/g, '<br>') +
          '</div>' +
          '</div>';
      }
      ...
      initialMsgHtml =
        '<div style="font-size:12px;color:var(--weak);padding:20px 0;text-align:center">暂无 AI 分析数据</div>';
    }
    ...
    var aiHtml =
      ...
      '<div class="dmsg agent"><div class="dmsg-bubble">' +
      initialMsgHtml +
      '</div></div>' +
      '</div>' +
    ...
    if (filterEl) {
      filterEl.onclick = function (e) {
        ...
        // 高亮当前按钮
        filterEl.querySelectorAll('.df-btn').forEach(function (b) {
  ...
  function generateAIAnswer(q, label, items, aiItems) {
    ...
    if (ql.indexOf('人手') > -1 || ql.indexOf('饱和') > -1 || ql.indexOf('人力') > -1) {
      ...
      for (var li = 0; li < lineNames.length; li++) {
        ...
        if (ln === '消防安全组') {
          parts.push(
            ln +
              '当前在岗4人，日均需要处理复查+新增约6.8项/人·天。参考行业标准4-5项/人·天，人力已超饱和36%-70%。建议优先从其他条线调配1-2名支援人员，或协商延期非紧急复查任务。',
          );
        } else if (ln === '企业安全组') {
          parts.push(
            ln +
              '当前在岗6人，日均处理约4.5项/人·天，处于合理范围。但5家异常主体集中在良渚片区，建议优化巡查路线避免跨片区耗时。',
          );
        } else {
    ...
    if (ql.indexOf('怎么') > -1 || ql.indexOf('建议') > -1 || ql.indexOf('解决') > -1 || ql.indexOf('措施') > -1) {
      ...
      if (lineNames.indexOf('消防安全组') > -1) {
        sug.push(
          '• 消防安全组：优先清理北苑商业综合体（超期最久，逾期3天）和云栖高层住宅，建议今日安排现场核查。同时排查复查人力饱和度问题，必要时申请临时增援。',
        );
      }
      if (lineNames.indexOf('企业安全组') > -1) {
        sug.push(
          '• 企业安全组：杭州恒源化工有限公司已有整改方案建议加快审批，杭州鑫盛机械制造有限公司和余杭天元纺织厂需从企业端推动——建议通知属地村社协助督促。',
        );
      }
  ...
  function openTaskDetail(taskName) {
    ...
    leftHtml +=
      ...
      statusCls +
      '">' +
      (task.status || task.type + '任务') +
      '</span>' +
      (task.risk !== '-' && task.risk
        ? '<span class="td-risk-tag high" style="background:color-mix(in oklch, ' +
          riskColor +
        ...
        : '') +
      (task.lag
        ? '<span style="font-size:10px;font-weight:600;color:var(--red);background:var(--red-soft);padding:2px 8px;border-radius:999px">滞后</span>'
        : '') +
      '</div>' +
      ...
      (task.lag ? '#dc2626' : rateNum >= 90 ? 'var(--green)' : '#d97706') +
      '"></div>' +
      '</div>' +
      ...
      '</span>' +
      '</div>' +
      '<div style="font-size:11px;color:var(--weak)">完成率 ' +
      task.rate +
      ' · 时间进度 ' +
      (task.progress || '-') +
      '</div>' +
    ...
    leftHtml +=
      ...
      ' 家</td></tr>' +
      '<tr><td>隐患总数</td><td>' +
      (task.hazards !== '-' ? task.hazards : '—') +
      ' 项</td></tr>' +
      '<tr><td>重大隐患</td><td>' +
      (task.majorHazards !== '-' ? task.majorHazards : '—') +
      ' 项</td></tr>' +
      '</table>' +
      '</div>';
    ...
    if (task.relatedItems && task.relatedItems.length > 0) {
      leftHtml += '<div class="task-detail-section">' + '<div class="task-detail-label">关联事项</div>';
      for (var ri = 0; ri < task.relatedItems.length; ri++) {
        var item = task.relatedItems[ri];
        var hazardName = item.split('·')[0].trim();
        leftHtml +=
          '<div class="td-related-item" class="pointer" onclick="closeTaskModal();openHazardDetail(\'' +
          hazardName.replace(/'/g, "\\'") +
          '\')">' +
    ...
    if (task.lag) {
      ...
      rightHtml +=
        ...
        '<div class="tma-item" style="font-weight:600;color:' +
        (task.statusCls === 'danger' ? 'var(--red)' : '#a75605') +
        '">⚠ ' +
        diagParts.join('，') +
        '</div>' +
        '</div>';
      ...
      rightHtml +=
        '<div class="tma-block" style="border-color:#c8e6c9;background:#f1faf5">' +
        '<div class="tma-label">AI 诊断</div>' +
    ...
    // 关联对象
    if (task.relatedItems && task.relatedItems.length > 0) {
      rightHtml += '<div class="tma-block">' + '<div class="tma-label">关联对象</div>';
      for (var ri = 0; ri < task.relatedItems.length; ri++) {
        var item = task.relatedItems[ri];
        var hazardName = item.split('·')[0].trim();
        rightHtml +=
          '<span class="tma-chip" onclick="closeTaskModal();openHazardDetail(\'' +
          hazardName.replace(/'/g, "\\'") +
          '\')">' +
          item +
          '</span>';
    ...
    rightHtml +=
      ...
      '<div class="tma-item"><span class="tma-dot ' +
      (task.lag ? 'red' : 'green') +
      '"></span>现场核查</div>' +
      '<div class="tma-item"><span class="tma-dot ' +
      (task.lag ? 'red' : 'green') +
  ...
  function switchScene(sceneId, force) {
    ...
    _switchTimer = setTimeout(function () {
      ...
      if (sceneId === 'rules') {
        ...
        if (chatBody) {
          chatBody.innerHTML +=
            '<div class="msg agent"><div class="bubble">已切换到「' +
            name +
      ...
      try {
        ...
        $dom.sceneContent.innerHTML = renderError(
          '渲染异常',
  ...
  function bindInteractions() {
    ...
    $dom.drawerConfirm.addEventListener('click', function () {
      ...
      if (action === 'supervise') {
        ...
        var resultHtml =
          '<div class="drawer-generated">' +
          '<div class="dr-gen-banner" style="background:var(--green-bg);color:var(--green)"><i data-lucide="check-circle" width="14" height="14" style="vertical-align:middle;margin-right:4px"></i> 已成功发起 ' +
          count +
          ' 条督办</div>' +
      ...
      var resultHtml =
        ...
        '<div class="dr-gen-text" id="drawerGenText">' +
        generated.replace(/\n/g, '<br>') +
        '</div>' +
        '<div class="dr-gen-actions">' +
        ...
        if (card) {
          ...
          if (!trackTitle && kind === 'ht') {
            ...
            if (row) {
              ...
              trackTitle =
                (cells[0] ? cells[0].textContent.trim() : '') + ' - ' + (cells[1] ? cells[1].textContent.trim() : '');
              YAQ.addTrack({
                title: trackTitle,
                source: '隐患整改日报',
                responsibility: cells[5] ? cells[5].textContent.trim() : '',
              });
            }
            e.stopPropagation();
            return;
  ...
  function sendChatMsg() {
    ...
    setTimeout(function () {
      chatBody.innerHTML +=
        '<div class="msg agent"><div class="bubble">已收到你的问题，正在分析「' + escapeHtml(text) + '」…</div></div>';
      chatBody.scrollTop = chatBody.scrollHeight;
    }, 400);
  }
  ...
  function renderSelectedMetrics(metrics) {
    ...
    for (var j = 0; j < checked.length; j++) {
      ...
      html +=
        ...
        m.value +
        (m.compare
          ? '<span class="mc-delta ' + (m.compare.isBad ? 'bad' : 'good') + '">' + m.compare.delta + '</span>'
          : '') +
        '</div>' +
        '<div class="mc-label">' +
        m.label +
  ...
  function renderMetricCheckboxes() {
    ...
    for (var f = 0; f < filterLabels.length; f++) {
      filterHtml +=
        '<button class="modal-filter-tab' +
        (filterLabels[f] === activeFilter ? ' active' : '') +
    ...
    filterHtml +=
      '<span style="width:1px;height:18px;background:var(--line);margin:0 6px;display:inline-block;vertical-align:middle"></span>';
    for (var pf = 0; pf < periodFilters.length; pf++) {
      filterHtml +=
        '<button class="modal-filter-tab' +
        (periodFilters[pf] === activePeriod ? ' active' : '') +
    ...
    for (var g = 0; g < groupOrder.length; g++) {
      ...
      if (!items) continue;
      html +=
        '<div style="font-size:11px;font-weight:600;color:var(--weak);margin-bottom:8px;margin-top:' +
        (g > 0 ? '14px' : '0') +
      ...
      for (var j = 0; j < items.length; j++) {
        ...
        html +=
          '<div class="metric-card' +
          alertCls +
          (m.checked ? ' mc-active' : ' mc-dim') +
          '" data-id="' +
          m.id +
          '" onclick="toggleMiniCard(this)" data-desc="' +
          (m.desc || '') +
          '" onmouseenter="showMetricTip(event,this.getAttribute(\'data-desc\'))" onmouseleave="hideMetricTip()">' +
    ...
    $dom.metricCheckboxes.innerHTML =
      html || '<div style="text-align:center;padding:30px 0;color:var(--weak);font-size:13px">该分组暂无指标</div>';
  ...
  function renderSelectedMetricsList() {
    ...
    for (var j = 0; j < checked.length; j++) {
      ...
      html +=
        '<div class="metric-card sel-card-drag' +
        alertCls2 +
        '" draggable="true" data-id="' +
        m.id +
        '" ondragstart="onDragStart(event)" ondragover="onDragOver(event)" ondrop="onDrop(event)" ondragend="onDragEnd(event)" data-desc="' +
        (m.desc || '') +
        '" onmouseenter="showMetricTip(event,this.getAttribute(\'data-desc\'))" onmouseleave="hideMetricTip()">' +
        (m.alert === 'danger'
          ...
          : m.alert === 'warning'
            ...
            : '') +
        '<span class="sel-hover-remove" onclick="event.stopPropagation();removeSelected(\'' +
        m.id +
        '\')"><i data-lucide="x" width="10" height="10"></i></span>' +
        '<div class="mc-value"' +
        (m.valueColor ? ' style="color:' + m.valueColor + '"' : '') +
        '>' +
        m.value +
  ...
  function onDragEnd(e) {
    document
      .querySelectorAll('.sel-card-drag.dragging, .sel-card-drag.drag-before, .sel-card-drag.drag-after')
      .forEach(function (c) {
  ...
  function showMetricTip(e, arg) {
    ...
    if (typeof arg === 'string') {
      ...
      if (compareStr) {
        try {
          ...
          html +=
            ...
            '<span class="mt-compare-value">' +
            compare.baselineLabel +
            ' ' +
            compare.baselineValue +
            '</span>' +
            '<span class="mt-delta ' +
            deltaCls +
            '">' +
        ...
        try {
          ...
          if (lineKeys.length > 0) {
            ...
            for (var li = 0; li < lineKeys.length; li++) {
              html +=
                '<span class="mt-dim-item"><span class="mt-dim-name">' +
                lineKeys[li] +
                '</span><span class="mt-dim-count">' +
                lineMap[lineKeys[li]] +
                '项</span></span>';
            }
            html += '</div>';
      ...
      // — 复制按钮 —
      html +=
        '<button class="mt-copy-btn" onclick="copyTipContent(event)"><i data-lucide="copy" width="11" height="11" aria-hidden="true"></i> 复制</button>';
    }
  ...
  function saveMetricConfig() {
    ...
    try {
      ...
      container.innerHTML =
        '<div class="error-state"><i data-lucide="alert-triangle" width="32" height="32" class="c-red"></i><h3>渲染异常</h3><p>指标配置已保存，但渲染场景时发生错误，请刷新页面。</p></div>';
    }
    showToast('指标配置已保存');
  }
  ...
  function buildSearchIndex() {
    ...
    for (var ei = 0; ei < MOCK.abnormalEvents.length; ei++) {
      ...
      if (ae.chain && ae.chain.responsible) {
        add(
          'person',
          'ae-' + ae.chain.responsible,
          ae.chain.responsible,
          '异常责任人',
  ...
  function renderSearchChips() {
    ...
    for (var i = 0; i < h.length && i < 10; i++) {
      ...
      html +=
        '<span class="l-chip" onclick="applySearchChip(\'' +
        jsSafe +
        '\')" title="搜索「' +
        $_escapeHtml(raw) +
        '」">' +
        $_escapeHtml(raw) +
        '<button class="l-chip-del" onmousedown="event.stopPropagation();removeSearchKeyword(\'' +
        jsSafe +
        '\')" title="移除">' +
  ...
  function renderLauncher() {
    ...
    if (query) {
      ...
      for (var _tgi = 0; _tgi < result.typeOrder.length; _tgi++) {
        ...
        html +=
          ...
          '<span class="sg-group-icon">' +
          g.icon +
          '</span>' +
          '<span class="sg-group-title">' +
          g.label +
          '</span>' +
          '<span class="sg-group-count">' +
          g.items.length +
          '</span>' +
    ...
    if (favs.length > 0) {
      ...
      if (favApps.length > 0) {
        html +=
          ...
          '<span class="launcher-group-count">' +
          favApps.length +
          '</span>' +
          '<span class="lgh-line"></span>' +
          '</div>' +
    ...
    for (var gi = 0; gi < LAUNCHER_DATA.length; gi++) {
      ...
      html +=
        '<div class="launcher-group">' +
        '<div class="launcher-group-head">' +
        '<span class="launcher-group-title">' +
        group.title +
        '</span>' +
        '<span class="launcher-group-count">' +
        group.apps.length +
  ...
  function renderSearchResultItem(entry, query) {
    var typeIconMap = {
      ...
      enterprise:
        '<div class="sri-icon sri-icon-enterprise"><i data-lucide="building-2" width="16" height="16"></i></div>',
    ...
    var iconHtml =
      typeIconMap[entry.type] || '<div class="sri-icon"><i data-lucide="search" width="16" height="16"></i></div>';
    ...
    return (
      '<div class="sri-item" onclick="executeSearchResult(\'' +
      $_escapeHtml(entry.id) +
      '\')">' +
      iconHtml +
      '<div class="sri-body">' +
      '<div class="sri-label">' +
  ...
  function buildLauncherItem(app, isFaved) {
    ...
    return (
      '<div class="launcher-item" onclick="launcherGo(\'' +
      app.id +
      '\')" title="' +
      (app.desc || '') +
      '">' +
      ...
      app.id +
      '\')" title="' +
      (isFaved || faved ? '取消收藏' : '收藏') +
      '">' +
      '<i data-lucide="star" width="13" height="13"></i>' +
      '</button>' +
      '<div class="li-icon"><i data-lucide="' +
      app.icon +
      '" width="22" height="22"></i></div>' +
      '<div class="li-name">' +
      app.name +
      '</div>' +
      '</div>'
  ...
  function renderTabs() {
    ...
    for (var ti = 0; ti < tabs.length; ti++) {
      ...
      var closeBtn =
        t.id === 'dashboard'
          ? ''
          : '<button class="tab-close" onclick="event.stopPropagation();YAQ.closeTab(\'' + t.id + '\')">✕</button>';
      html +=
        '<div class="tab-item' +
        active +
        '" onclick="YAQ.switchTab(\'' +
        t.id +
        '\')">' +
        t.label +
        closeBtn +
  ...
  Object.assign(window.YAQ, {
    ...
    // ─── 处置建议 ───
    copyDisposalRec: function (idx) {
      YAQ.copyDisposalRec(idx);
    },
    generateAllDisposalText: function () {
      YAQ.generateAllDisposalText();
    },
    regenerateDisposalRecs: function () {
      YAQ.regenerateDisposalRecs();
    },
    copyDrawerGenerated: function () {
      YAQ.copyDrawerGenerated();
    },
  ...
  YAQ.resetAndCloseMenu = function () {
    window.resetInit();
    window.closeDemoMenu();
  };
  YAQ.dashboardRedirectAndClose = function () {
    window.doDashboardRedirect();
    window.closeDemoMenu();
  };
  YAQ.normalDashboardAndClose = function () {
    window.doNormalDashboard();
    window.closeDemoMenu();
  };
  ...
  document.addEventListener('click', function (e) {
    ...
    if (el) {
      switchScene(el.getAttribute('data-scene'));
      return;
    }
  ...
  function changePendingAction(paId, newType) {
    ...
    if (modal && modal.classList.contains('open')) {
      if (window.openActionConfirmation) window.openActionConfirmation();
      return;
    }
    renderScene('pending-actions');
  }
  ...
  function togglePASelection(paId, checked) {
    ...
    if (card) {
      if (checked) {
        card.classList.add('pa-card-selected');
      } else {
        card.classList.remove('pa-card-selected');
      }
    }
    // 更新确认弹窗中的计数
    ...
    if (countEl) {
      var n = 0;
      for (var k in state.selectedPAIds) {
        if (state.selectedPAIds.hasOwnProperty(k)) n++;
      }
  ...
  // ════════════════════════════════════════════════════════════════
  // 智能专项检查（Special Inspection）场景
  // — 页面风格与其他场景一致，无聊天气泡/头像
  // — 点击「开始模拟」按钮自动演示完整流程
  // ════════════════════════════════════════════════════════════════

  var SI_DOC_NAME = '关于开展2026年第三季度危化品使用企业专项检查的通知.pdf';

  var SI_TASKS = [
    { id: 1, name: '危险化学品储罐区安全设施检查', target: '危化品使用企业', standard: '对照《危险化学品安全管理条例》', priority: '高优', person: '李明', deadline: '7月10日', checked: true },
    { id: 2, name: '防雷防静电设施检测报告核查', target: '危化品使用企业', standard: '防雷防静电设施检测报告', priority: '高优', person: '李明', deadline: '7月12日', checked: true },
    { id: 3, name: '化学品安全技术说明书（SDS）检查', target: '危化品使用企业', standard: 'SDS 完整性及更新情况', priority: '中', person: '范嘉杰', deadline: '7月15日', checked: true },
    { id: 4, name: '应急演练记录及预案评估', target: '工贸企业', standard: '应急演练频次及预案有效性', priority: '中', person: '范嘉杰', deadline: '7月18日', checked: false },
    { id: 5, name: '危废暂存间规范检查', target: '工贸企业', standard: '危废标识、台账、储存规范', priority: '中', person: '李明', deadline: '7月20日', checked: false },
    { id: 6, name: '有限空间作业审批流程检查', target: '工贸企业', standard: '有限空间作业审批及监护制度', priority: '低', person: '范嘉杰', deadline: '7月25日', checked: false },
  ];

  var SI_STEPS = [
    { label: '📄 文档分类', detail: '识别为危化品专项检查通知（2026年第三季度）', conf: '98%' },
    { label: '📋 检查要点提取', detail: '提取 4 大类 16 项检查要点（危化品储罐 5 项、防雷防静电 3 项、SDS 4 项、应急管理 4 项）', conf: '95%' },
    { label: '🎯 对象范围识别', detail: '覆盖良渚街道 23 家相关企业（危化品使用企业 8 家、工贸企业 15 家）', conf: '92%' },
    { label: '✅ 任务生成', detail: '拆解为 6 项可执行检查任务，已匹配负责人和期限', conf: '90%' },
  ];

  // ─── 渲染入口 ──────────────────────────────────────────────

  function renderSpecialInspection() {
    var html =
      ...
      '<p style="font-size:13px;color:var(--weak);margin-bottom:16px;line-height:1.6">' +
        '模拟 AI 解析专项检查通知文件并自动生成检查任务的完整流程。点击下方按钮开始演示。' +
      '</p>' +
  ...
  YAQ.siStartDemo = function () {
    ...
    content.innerHTML =
      '<div class="info-card" id="siStepParsing" style="animation:si-fadeSlideIn 0.4s ease both">' +
        '<div class="info-card-head"><h3><i data-lucide="file-text" width="16" height="16" style="color:var(--accent)"></i> 📄 文档解析中</h3></div>' +
        '<div style="padding:8px 0;font-size:13px;color:var(--weak)">' +
          '正在扫描文档：<strong>' + SI_DOC_NAME + '</strong>' +
        '</div>' +
        '<div style="margin:8px 0">' +
          '<div style="display:flex;flex-direction:column;gap:6px;padding:4px 0" id="siThinkSteps"></div>' +
        '</div>' +
      '</div>';
    ...
    // Step 2: 解析完成 → 确认卡片
    setTimeout(function () { YAQ.siShowParseResult(); }, SI_STEPS.length * 700 + 600);
  };

  YAQ.siShowParseResult = function () {
    var content = document.getElementById('siDemoContent');
    if (!content) return;

    // Replace parsing card with result
    content.innerHTML =
      '<div class="info-card" style="animation:si-fadeSlideIn 0.4s ease both">' +
        '<div class="info-card-head">' +
          '<h3><i data-lucide="check-circle" width="16" height="16" style="color:var(--green)"></i> ✅ 文档解析完成</h3>' +
          '<span class="info-card-badge" style="background:var(--green);color:#fff">解析成功</span>' +
        '</div>' +
        '<div style="padding:8px 0;font-size:13px;line-height:1.7">' +
          '已识别文件：<strong>' + SI_DOC_NAME + '</strong><br>' +
          '检查类型：危化品专项检查 | 覆盖企业：23 家 | 检查项：16 项' +
        '</div>' +
        '<div style="display:flex;gap:12px;margin:8px 0 12px">' +
          '<div style="flex:1;text-align:center;padding:8px;background:var(--accent-light);border-radius:8px">' +
            '<div style="font-size:22px;font-weight:700;color:var(--accent)">23</div>' +
            '<div style="font-size:11px;color:var(--weak)">覆盖企业</div>' +
          '</div>' +
          '<div style="flex:1;text-align:center;padding:8px;background:var(--accent-light);border-radius:8px">' +
            '<div style="font-size:22px;font-weight:700;color:var(--accent)">16</div>' +
            '<div style="font-size:11px;color:var(--weak)">检查要点</div>' +
          '</div>' +
          '<div style="flex:1;text-align:center;padding:8px;background:var(--accent-light);border-radius:8px">' +
            '<div style="font-size:22px;font-weight:700;color:var(--accent)">6</div>' +
            '<div style="font-size:11px;color:var(--weak)">待生成任务</div>' +
          '</div>' +
        '</div>' +
        '<div style="display:flex;gap:8px">' +
          '<button class="si-btn si-btn-primary" onclick="YAQ.siShowTasks()">✅ 确认，生成检查任务</button>' +
        '</div>' +
      '</div>';
    lucide.createIcons({ scope: content });
    var ws = document.getElementById('workspace');
  ...
  YAQ.siShowTasks = function () {
    var content = document.getElementById('siDemoContent');
    if (!content) return;

    var taskHtml = '<div style="margin:4px 0">';
    for (var i = 0; i < SI_TASKS.length; i++) {
      var t = SI_TASKS[i];
      var tagClass = t.priority === '高优' ? 'si-tag-high' : t.priority === '中' ? 'si-tag-mid' : 'si-tag-low';
      taskHtml +=
        '<div style="display:flex;align-items:flex-start;gap:10px;padding:10px 14px;background:var(--surface);border:1px solid var(--line);border-radius:8px;margin-bottom:6px">' +
          '<input type="checkbox" style="margin-top:2px;width:16px;height:16px;accent-color:var(--accent);cursor:pointer" ' + (t.checked ? 'checked' : '') + ' onchange="YAQ.siToggleTask(' + t.id + ')">' +
          '<div style="flex:1">' +
            '<div style="font-weight:600;font-size:13px;margin-bottom:2px"><span class="si-tag ' + tagClass + '">' + t.priority + '</span> ' + escapeHtml(t.name) + '</div>' +
            '<div style="font-size:11px;color:var(--weak);display:flex;flex-wrap:wrap;gap:6px">' +
              '<span><i data-lucide="building-2" width="10" height="10"></i> ' + escapeHtml(t.target) + '</span>' +
              '<span><i data-lucide="clipboard-list" width="10" height="10"></i> ' + escapeHtml(t.standard) + '</span>' +
              '<span><i data-lucide="user" width="10" height="10"></i> ' + escapeHtml(t.person) + '</span>' +
              '<span><i data-lucide="calendar" width="10" height="10"></i> ' + escapeHtml(t.deadline) + '</span>' +
            '</div>' +
          '</div>' +
          '<button class="si-btn si-btn-outline si-btn-sm" onclick="YAQ.siOpenTaskEdit(' + t.id + ')" style="flex-shrink:0;padding:3px 8px;font-size:11px">✏️</button>' +
        '</div>';
    }
    taskHtml += '</div>';
    ...
    content.innerHTML +=
      '<div class="info-card" style="animation:si-fadeSlideIn 0.4s ease both;margin-top:12px">' +
        '<div class="info-card-head">' +
          '<h3><i data-lucide="list-todo" width="16" height="16" style="color:var(--accent)"></i> 📋 检查任务列表</h3>' +
          '<span class="info-card-badge" style="background:var(--accent);color:#fff">' + SI_TASKS.length + ' 项</span>' +
        '</div>' +
        '<div style="font-size:12px;color:var(--weak);margin-bottom:8px">AI 根据文件内容拆解出以下检查任务。勾选或编辑后确认：</div>' +
        taskHtml +
        '<div style="display:flex;gap:8px;margin-top:8px">' +
          '<button class="si-btn si-btn-success" onclick="YAQ.siShowSchedule()">✅ 确认任务，生成排期</button>' +
        '</div>' +
      '</div>';
    lucide.createIcons({ scope: content });
    var ws = document.getElementById('workspace');
  ...
  YAQ.siToggleTask = function (id) {
    for (var i = 0; i < SI_TASKS.length; i++) {
      if (SI_TASKS[i].id === id) { SI_TASKS[i].checked = !SI_TASKS[i].checked; break; }
    }
  };

  YAQ.siShowSchedule = function () {
    var content = document.getElementById('siDemoContent');
    if (!content) return;
    ...
    content.innerHTML +=
      '<div class="info-card" style="animation:si-fadeSlideIn 0.4s ease both;margin-top:12px">' +
        '<div class="info-card-head">' +
          '<h3><i data-lucide="calendar-range" width="16" height="16" style="color:var(--accent)"></i> 📊 执行计划</h3>' +
          '<span class="info-card-badge" style="background:var(--orange);color:#fff">待确认</span>' +
        '</div>' +
        '<div style="font-size:12px;color:var(--weak);margin-bottom:12px">任务已按优先级和人员负荷分配排期：</div>' +
        '<div style="display:flex;align-items:center;gap:2px;height:28px;margin-bottom:12px">' +
          '<div style="height:100%;border-radius:3px;flex:1.5;display:flex;align-items:center;justify-content:center;font-size:9px;color:#fff;font-weight:600;background:#2563eb">李明 - 危化</div>' +
          '<div style="height:100%;border-radius:3px;flex:1;display:flex;align-items:center;justify-content:center;font-size:9px;color:#fff;font-weight:600;background:#7c3aed">李明 - 工贸</div>' +
          '<div style="height:100%;border-radius:3px;flex:1;display:flex;align-items:center;justify-content:center;font-size:9px;color:#fff;font-weight:600;background:#059669">范嘉杰 - 危化</div>' +
          '<div style="height:100%;border-radius:3px;flex:1.5;display:flex;align-items:center;justify-content:center;font-size:9px;color:#fff;font-weight:600;background:#d97706">范嘉杰 - 工贸</div>' +
        '</div>' +
        '<div style="font-size:12px;line-height:1.7;margin-bottom:12px">' +
          '📅 时间范围：<strong>7月1日 - 7月30日</strong>（共 30 天）<br>' +
          '👤 执行人员：李明（3 项）、范嘉杰（3 项）<br>' +
          '🎯 覆盖企业：23 家（危化 8 家 · 工贸 15 家）' +
        '</div>' +
        '<div style="display:flex;gap:8px">' +
  ...
  YAQ.siShowResult = function () {
    ...
    var highCount = 0;
    for (var i = 0; i < SI_TASKS.length; i++) {
      if (SI_TASKS[i].checked) checkedCount++;
      if (SI_TASKS[i].checked && SI_TASKS[i].priority === '高优') highCount++;
    }
    ...
    content.innerHTML +=
      '<div class="info-card" style="animation:si-fadeSlideIn 0.4s ease both;margin-top:12px;border-color:#bbf7d0">' +
        ...
        '</div>' +
        '<div style="font-size:12px;color:var(--weak);margin-bottom:12px">任务已生成并下发，共 ' + checkedCount + ' 项任务进入执行阶段。</div>' +
        '<div style="display:grid;grid-template-columns:repeat(2,1fr);gap:12px;margin-bottom:8px">' +
          '<div style="background:var(--surface);border:1px solid var(--line);border-radius:10px;padding:14px 16px">' +
            '<div style="font-size:12px;font-weight:600;color:var(--weak);margin-bottom:4px">📋 进行中任务</div>' +
            '<div style="font-size:22px;font-weight:700">' + checkedCount + '</div>' +
          ...
          '</div>' +
          '<div style="background:var(--surface);border:1px solid var(--line);border-radius:10px;padding:14px 16px">' +
            '<div style="font-size:12px;font-weight:600;color:var(--weak);margin-bottom:4px">👤 执行人员</div>' +
            '<div style="font-size:22px;font-weight:700">2 人</div>' +
            '<div style="font-size:11px;color:var(--weak);margin-top:2px">李明 · 范嘉杰</div>' +
          '</div>' +
          '<div style="background:var(--surface);border:1px solid var(--line);border-radius:10px;padding:14px 16px">' +
            '<div style="font-size:12px;font-weight:600;color:var(--weak);margin-bottom:4px">🎯 覆盖企业</div>' +
            '<div style="font-size:22px;font-weight:700">23 家</div>' +
            '<div style="font-size:11px;color:var(--weak);margin-top:2px">危化 8 家 · 工贸 15 家</div>' +
          '</div>' +
        '</div>' +
  ...
  YAQ.siOpenTaskEdit = function (id) {
    var t = null;
    for (var i = 0; i < SI_TASKS.length; i++) {
      if (SI_TASKS[i].id === id) { t = SI_TASKS[i]; break; }
    }
    if (!t) return;

    var persons = ['李明', '范嘉杰', '王磊', '张毅'];
    var personOpts = '';
    for (var pi = 0; pi < persons.length; pi++) {
      personOpts += '<option' + (t.person === persons[pi] ? ' selected' : '') + '>' + persons[pi] + '</option>';
    }

    document.getElementById('siModalBody').innerHTML =
      '<div style="margin-bottom:16px">' +
        '<label>任务名称</label>' +
        '<textarea id="siEditName" rows="2">' + escapeHtml(t.name) + '</textarea>' +
      '</div>' +
      '<div style="margin-bottom:16px">' +
        '<label>检查对象</label>' +
        '<input id="siEditTarget" value="' + escapeHtml(t.target) + '">' +
      '</div>' +
      '<div style="margin-bottom:16px">' +
        '<label>检查标准</label>' +
        '<input id="siEditStandard" value="' + escapeHtml(t.standard) + '">' +
      '</div>' +
      '<div style="margin-bottom:16px">' +
        '<label>负责人</label>' +
        '<select id="siEditPerson">' + personOpts + '</select>' +
      '</div>' +
      '<div style="margin-bottom:8px">' +
        '<label>期限</label>' +
        '<input id="siEditDeadline" value="' + escapeHtml(t.deadline) + '">' +
      '</div>' +
      '<div class="si-modal-actions">' +
        '<button class="si-btn si-btn-outline" onclick="YAQ.siCloseTaskEdit()">取消</button>' +
        '<button class="si-btn si-btn-primary" onclick="YAQ.siSaveTaskEdit()">保存</button>' +
      '</div>';

    document.getElementById('siModalOverlay').classList.add('active');
  };

  YAQ.siCloseTaskEdit = function () {
    document.getElementById('siModalOverlay').classList.remove('active');
  };

  YAQ.siSaveTaskEdit = function () {
    var t = null;
    var editIdx = -1;
    var name = document.getElementById('siEditName')?.value;
    var target = document.getElementById('siEditTarget')?.value;
    var standard = document.getElementById('siEditStandard')?.value;
    var person = document.getElementById('siEditPerson')?.value;
    var deadline = document.getElementById('siEditDeadline')?.value;
    if (!name) { showToast('请填写任务名称'); return; }

    // Find task by matching name (simple approach)
    for (var i = 0; i < SI_TASKS.length; i++) {
      if (SI_TASKS[i].name === name || SI_TASKS[i].name.indexOf(name.substring(0, 4)) >= 0) {
        t = SI_TASKS[i]; editIdx = i; break;
      }
    }
    if (!t) { showToast('请先关闭弹窗后重试'); }
    if (t) {
      t.name = name;
      t.target = target || t.target;
      t.standard = standard || t.standard;
      t.person = person || t.person;
  ...
  YAQ.siRefreshTasks = function () {
    var content = document.getElementById('siDemoContent');
    if (!content) return;
    // Find the task card and rebuild it
    var taskCards = content.querySelectorAll('.info-card');
    for (var ci = 0; ci < taskCards.length; ci++) {
      var card = taskCards[ci];
      if (card.querySelector('.si-task-item')) {
        // This is the task card - rebuild its task items
        var listContainer = card.querySelector('[style*="margin:4px 0"]') || card.querySelector('div:first-child');
        if (listContainer) {
          var taskHtml = '';
          for (var i = 0; i < SI_TASKS.length; i++) {
            var t = SI_TASKS[i];
